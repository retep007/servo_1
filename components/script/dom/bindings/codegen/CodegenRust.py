...
def getJSToNativeConversionInfo(type, descriptorProvider, failureCode=None,
    ...
    if type.isDictionary():
        ...
        empty = "%s::%s::empty(cx)" % (CGDictionary.makeModuleName(type.inner), CGDictionary.makeDictionaryName(type.inner))
...
class CGDictionary(CGThing):
    def __init__(self, dictionary, descriptorProvider):
        ...
        self.isGeneric = dictionary.isGenericOverTypeHolder()
    ...
    def struct(self):
        ...
        return (string.Template(
                ...
                "}").substitute({"selfName": name,
                                 ...
                                 "mustRoot": mustRoot,
                                 "derive": ', '.join(derive),
                                 "typeParams": MaybeTypeHolded(self.isGeneric).getTypeParams()}))
    ...
    def impl(self):
        ...
        typeHolded = MaybeTypeHolded(self.isGeneric)
...
class GlobalGenRoots():
    ...
    def InterfaceObjectMapData(config):
        ...
        for d in config.getDescriptors(hasInterfaceObject=True, isInline=False):
            ...
            pairs.append((d.name, binding, binding, ('::<TH>')))
            for ctor in d.interface.namedConstructors:
                pairs.append((ctor.identifier.name, binding, binding, ('::<TH>')))
