...
pub use self::TestWorkletBinding::{Wrap, TestWorkletMethods, GetProtoObject, DefineDOMInterface};
...
(cx: *mut JSContext, _obj: HandleObject, this: *const TestWorklet<TH>, args: *const JSJitMethodCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        if argc < 1 {
            throw_type_error(cx, "Not enough arguments to \"TestWorklet.addModule\".");
            return false;
        }
...
unsafe extern fn lookup<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, _obj: HandleObject, this: *const TestWorklet<TH>, args: *const JSJitMethodCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        let this = &*this;
        let args = &*args;
        ...
        if argc < 1 {
            throw_type_error(cx, "Not enough arguments to \"TestWorklet.lookup\".");
            return false;
        }
        ...
        let result: Option<DOMString> = this.Lookup(arg0);
...
fn lookup_methodinfo<TH: TypeHolderTrait<TH>>() -> JSJitInfo { JSJitInfo {
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<TestWorklet<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut TestWorklet<TH>);
            }
            debug!("TestWorklet<TH> finalize: {:p}", this);
    }), ());
}
...
fn Class<TH: TypeHolderTrait<TH>>() -> DOMJSClass { DOMJSClass {
    base: js::jsapi::JSClass {
        name: b"TestWorklet\0" as *const u8 as *const libc::c_char,
    ...
    dom_class: DOMClass {
    interface_chain: [ PrototypeList::ID::TestWorklet, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last ],
    type_id: ::dom::bindings::codegen::InheritTypes::TopTypeId { alone: () },
    global: InterfaceObjectMap::Globals::EMPTY,
    malloc_size_of: malloc_size_of_including_raw_self::<TestWorklet<TH>> as unsafe fn(&mut _, _) -> _,
}
}}

#[inline]
fn malloc_size<TH: TypeHolderTrait<TH>>(ops: &mut MallocSizeOfOps, obj: *const c_void) -> usize {
    malloc_size_of_including_raw_self::<TestWorklet<TH>>(ops, obj)
}

pub unsafe fn Wrap<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<TestWorklet<TH>>) -> DomRoot<TestWorklet<TH>> {
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for TestWorklet<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait<TH>> PartialEq for TestWorklet<TH> {
    fn eq(&self, other: &TestWorklet<TH>) -> bool {
        self as *const TestWorklet<TH> == &*other
    }
}

pub trait TestWorkletMethods<TH: TypeHolderTrait<TH>> {
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::TestWorklet as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"TestWorkletPrototype\0" as *const u8 as *const libc::c_char,
    flags:
...
fn INTERFACE_OBJECT_CLASS<TH: TypeHolderTrait<TH>>() -> NonCallbackInterfaceObjectClass {
    ...
    NonCallbackInterfaceObjectClass::new(
        ...
        b"function TestWorklet() {\n    [native code]\n}",
        PrototypeList::ID::TestWorklet,
        0) 
}
...
(aCx: *mut JSContext, aObj: HandleObject) -> bool {
    ...
    PREFS.get("dom.worklet.testing.enabled").as_boolean().unwrap_or(false)
}
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    assert!((*cache)[PrototypeList::ID::TestWorklet as usize].is_null());
    (*cache)[PrototypeList::ID::TestWorklet as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::TestWorklet as isize),
                                  ptr::null_mut(),
                                  prototype.get());
