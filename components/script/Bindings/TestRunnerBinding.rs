...
pub use self::TestRunnerBinding::{Wrap, TestRunnerMethods, GetProtoObject, DefineDOMInterface};
...
unsafe extern fn setBluetoothMockDataSet<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, _obj: HandleObject, this: *const TestRunner<TH>, args: *const JSJitMethodCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        if argc < 1 {
            throw_type_error(cx, "Not enough arguments to \"TestRunner.setBluetoothMockDataSet\".");
            return false;
        }
...
fn setBluetoothMockDataSet_methodinfo<TH: TypeHolderTrait<TH>>() -> JSJitInfo { JSJitInfo {
    call: setBluetoothMockDataSet::<TH> as *const os::raw::c_void,
    protoID: PrototypeList::ID::TestRunner as u16,
    depth: 0,
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<TestRunner<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut TestRunner<TH>);
            }
            debug!("TestRunner<TH> finalize: {:p}", this);
    }), ());
}
...
fn Class<TH: TypeHolderTrait<TH>>() -> DOMJSClass { DOMJSClass {
    ...
    dom_class: DOMClass {
    interface_chain: [ PrototypeList::ID::TestRunner, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last ],
    type_id: ::dom::bindings::codegen::InheritTypes::TopTypeId { alone: () },
    global: InterfaceObjectMap::Globals::EMPTY,
    malloc_size_of: malloc_size_of_including_raw_self::<TestRunner<TH>> as unsafe fn(&mut _, _) -> _,
}
}}

#[inline]
fn malloc_size<TH: TypeHolderTrait<TH>>(ops: &mut MallocSizeOfOps, obj: *const c_void) -> usize {
    malloc_size_of_including_raw_self::<TestRunner<TH>>(ops, obj)
}

pub unsafe fn Wrap<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<TestRunner<TH>>) -> DomRoot<TestRunner<TH>> {
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for TestRunner<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait<TH>> PartialEq for TestRunner<TH> {
    fn eq(&self, other: &TestRunner<TH>) -> bool {
        self as *const TestRunner<TH> == &*other
    }
}

pub trait TestRunnerMethods<TH: TypeHolderTrait<TH>> {
...
&[
    JSFunctionSpec {
        name: b"setBluetoothMockDataSet\0" as *const u8 as *const libc::c_char,
        call: JSNativeWrapper { op: Some(generic_method), info: &setBluetoothMockDataSet_methodinfo::<TH>() as *const _ as *const JSJitInfo },
        nargs: 1,
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::TestRunner as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"TestRunnerPrototype\0" as *const u8 as *const libc::c_char,
    flags:
...
static INTERFACE_OBJECT_CLASS: NonCallbackInterfaceObjectClass =
    NonCallbackInterfaceObjectClass::new(
        ...
        b"function TestRunner() {\n    [native code]\n}",
        PrototypeList::ID::TestRunner,
        0);
...
(aCx: *mut JSContext, aObj: HandleObject) -> bool {
    ...
    PREFS.get("dom.bluetooth.testing.enabled").as_boolean().unwrap_or(false)
}
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    assert!((*cache)[PrototypeList::ID::TestRunner as usize].is_null());
    (*cache)[PrototypeList::ID::TestRunner as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::TestRunner as isize),
                                  ptr::null_mut(),
                                  prototype.get());
