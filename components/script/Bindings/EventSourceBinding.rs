...
impl EventSourceInit {
    pub unsafe fn empty(cx: *mut JSContext) -> EventSourceInit {
        match EventSourceInit::new(cx, HandleValue::null()) {
            Ok(ConversionResult::Success(v)) => v,
            _ => unreachable!(),
        }
    }
    pub unsafe fn new(cx: *mut JSContext, val: HandleValue)
                      -> Result<ConversionResult<EventSourceInit>, ()> {
        ...
        rooted!(in(cx) let object = object);
            let dictionary = EventSourceInit {
            withCredentials: {
                rooted!(in(cx) let mut rval = UndefinedValue());
                match try!(get_dictionary_property(cx, object.handle(), "withCredentials", rval.handle_mut())) {
                    true => {
...
impl ToJSValConvertible for EventSourceInit {
    unsafe fn to_jsval(&self, cx: *mut JSContext, mut rval: MutableHandleValue) {
        ...
        let withCredentials = &self.withCredentials;
        rooted!(in(cx) let mut withCredentials_js = UndefinedValue());
        withCredentials.to_jsval(cx, withCredentials_js.handle_mut());
        set_dictionary_property(cx, obj.handle(), "withCredentials", withCredentials_js.handle()).unwrap();
        rval.set(ObjectOrNullValue(obj.get()))
    }
}


pub use self::EventSourceBinding::{EventSourceConstants, Wrap, EventSourceMethods, GetProtoObject, DefineDOMInterface};
...
(cx: *mut JSContext, _obj: HandleObject, this: *const EventSource<TH>, args: JSJitGetterCallArgs) -> bool {
...
fn url_getterinfo<TH: TypeHolderTrait<TH>>() -> JSJitInfo { JSJitInfo {
    ...
    protoID: PrototypeList::ID::EventSource as u16,
    depth: 1,
...
(cx: *mut JSContext, obj: HandleObject, this: *const EventSource<TH>, args: JSJitSetterCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        let this = &*this;
...
(cx: *mut JSContext, _obj: HandleObject, this: *const EventSource<TH>, args: *const JSJitMethodCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        let this = &*this;
        let args = &*args;
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<EventSource<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut EventSource<TH>);
            }
            debug!("EventSource<TH> finalize: {:p}", this);
    }), ());
}
...
fn Class<TH: TypeHolderTrait<TH>>() -> DOMJSClass { DOMJSClass {
    base: js::jsapi::JSClass {
        name: b"EventSource\0" as *const u8 as *const libc::c_char,
    ...
    dom_class: DOMClass {
    interface_chain: [ PrototypeList::ID::EventTarget, PrototypeList::ID::EventSource, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last ],
    type_id: ::dom::bindings::codegen::InheritTypes::TopTypeId { eventtarget: (::dom::bindings::codegen::InheritTypes::EventTargetTypeId::EventSource) },
    global: InterfaceObjectMap::Globals::EMPTY,
    malloc_size_of: malloc_size_of_including_raw_self::<EventSource<TH>> as unsafe fn(&mut _, _) -> _,
}
}}

#[inline]
fn malloc_size<TH: TypeHolderTrait<TH>>(ops: &mut MallocSizeOfOps, obj: *const c_void) -> usize {
    malloc_size_of_including_raw_self::<EventSource<TH>>(ops, obj)
}

pub unsafe fn Wrap<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<EventSource<TH>>) -> DomRoot<EventSource<TH>> {
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for EventSource<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait<TH>> PartialEq for EventSource<TH> {
    fn eq(&self, other: &EventSource<TH>) -> bool {
        self as *const EventSource<TH> == &*other
    }
}

pub trait EventSourceMethods<TH: TypeHolderTrait<TH>> {
...
&[
    ...
    JSPropertySpec {
        ...
        flags: (JSPROP_ENUMERATE | JSPROP_SHARED) as u8,
        getter: JSNativeWrapper { op: Some(generic_getter), info: &withCredentials_getterinfo::<TH>() },
        setter: JSNativeWrapper { op: None, info: 0 as *const JSJitInfo }
    },
    JSPropertySpec {
...
&[
    ...
    ConstantSpec { name: b"CLOSED\0", value: ConstantVal::IntVal(2) }]
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::EventSource as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"EventSourcePrototype\0" as *const u8 as *const libc::c_char,
    flags:
...
(cx: *mut JSContext, argc: u32, vp: *mut JSVal) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        if argc < 1 {
            throw_type_error(cx, "Not enough arguments to \"EventSource.constructor\".");
            return false;
        }
        ...
        let arg1: dom::bindings::codegen::Bindings::EventSourceBinding::EventSourceInit = if args.get(1).is_undefined() {
            dom::bindings::codegen::Bindings::EventSourceBinding::EventSourceInit::empty(cx)
        } else {
        ...
        let result: Result<DomRoot<EventSource<TH>>, Error<TH>> = EventSource::Constructor(&global, arg0, &arg1);
...
fn INTERFACE_OBJECT_CLASS<TH: TypeHolderTrait<TH>>() -> NonCallbackInterfaceObjectClass {
    ...
    NonCallbackInterfaceObjectClass::new(
        ...
        b"function EventSource() {\n    [native code]\n}",
        PrototypeList::ID::EventSource,
        1) 
}
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    assert!((*cache)[PrototypeList::ID::EventSource as usize].is_null());
    (*cache)[PrototypeList::ID::EventSource as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::EventSource as isize),
                                  ptr::null_mut(),
                                  prototype.get());
