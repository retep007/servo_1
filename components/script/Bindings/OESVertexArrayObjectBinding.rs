...
pub use self::OESVertexArrayObjectBinding::{OESVertexArrayObjectConstants, Wrap, OESVertexArrayObjectMethods, GetProtoObject};
...
use dom::bindings::weakref::WeakReferenceable;
use dom::globalscope::GlobalScope;
use dom::types::OESVertexArrayObject;
use dom::types::WebGLObject;
use dom::types::WebGLVertexArrayObjectOES;
use dom::windowproxy::WindowProxy;
use js;
use js::JSCLASS_GLOBAL_SLOT_COUNT;
use js::JSCLASS_IS_DOMJSCLASS;
...
(cx: *mut JSContext, _obj: HandleObject, this: *const OESVertexArrayObject<TH>, args: *const JSJitMethodCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let result: Option<DomRoot<WebGLVertexArrayObjectOES<TH>>> = this.CreateVertexArrayOES();
...
(cx: *mut JSContext, _obj: HandleObject, this: *const OESVertexArrayObject<TH>, args: *const JSJitMethodCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        if argc < 1 {
            throw_type_error(cx, "Not enough arguments to \"OESVertexArrayObject.deleteVertexArrayOES\".");
            return false;
        }
        let arg0: Option<DomRoot<WebGLVertexArrayObjectOES<TH>>> = if args.get(0).get().is_object() {
            Some(match root_from_handlevalue(args.get(0)) {
                Ok(val) => val,
                Err(()) => {
                    throw_type_error(cx, "value does not implement interface WebGLVertexArrayObjectOES.");
                    return false;
        ...
        let result: () = this.DeleteVertexArrayOES(arg0.r());
...
(cx: *mut JSContext, _obj: HandleObject, this: *const OESVertexArrayObject<TH>, args: *const JSJitMethodCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        if argc < 1 {
            throw_type_error(cx, "Not enough arguments to \"OESVertexArrayObject.isVertexArrayOES\".");
            return false;
        }
        let arg0: Option<DomRoot<WebGLVertexArrayObjectOES<TH>>> = if args.get(0).get().is_object() {
            Some(match root_from_handlevalue(args.get(0)) {
        ...
        let result: bool = this.IsVertexArrayOES(arg0.r());
...
(cx: *mut JSContext, _obj: HandleObject, this: *const OESVertexArrayObject<TH>, args: *const JSJitMethodCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        if argc < 1 {
            throw_type_error(cx, "Not enough arguments to \"OESVertexArrayObject.bindVertexArrayOES\".");
            return false;
        }
        let arg0: Option<DomRoot<WebGLVertexArrayObjectOES<TH>>> = if args.get(0).get().is_object() {
            Some(match root_from_handlevalue(args.get(0)) {
        ...
        let result: () = this.BindVertexArrayOES(arg0.r());
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<OESVertexArrayObject<TH>, TH>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut OESVertexArrayObject<TH>);
            }
            debug!("OESVertexArrayObject<TH> finalize: {:p}", this);
    }), ());
}
...
fn Class<TH: TypeHolderTrait>() -> DOMJSClass { DOMJSClass {
    ...
    dom_class: DOMClass {
    interface_chain: [ PrototypeList::ID::OESVertexArrayObject, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last ],
    type_id: ::dom::bindings::codegen::InheritTypes::TopTypeId { alone: () },
    global: InterfaceObjectMap::Globals::EMPTY,
    malloc_size_of: malloc_size_of_including_raw_self::<OESVertexArrayObject<TH>> as unsafe fn(&mut _, _) -> _,
}
}}
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::OESVertexArrayObject as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"OESVertexArrayObjectPrototype\0" as *const u8 as *const libc::c_char,
    flags:
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    assert!((*cache)[PrototypeList::ID::OESVertexArrayObject as usize].is_null());
    (*cache)[PrototypeList::ID::OESVertexArrayObject as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::OESVertexArrayObject as isize),
                                  ptr::null_mut(),
                                  prototype.get());

}
} // mod OESVertexArrayObjectBinding
