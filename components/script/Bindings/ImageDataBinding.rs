...
pub use self::ImageDataBinding::{Wrap, ImageDataMethods, GetProtoObject, DefineDOMInterface};
...
(cx: *mut JSContext, _obj: HandleObject, this: *const ImageData<TH>, args: JSJitGetterCallArgs) -> bool {
...
(cx: *mut JSContext, _obj: HandleObject, this: *const ImageData<TH>, args: JSJitGetterCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        let this = &*this;
        let result: NonNull<JSObject> = this.Data(cx);
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<ImageData<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut ImageData<TH>);
            }
            debug!("ImageData<TH> finalize: {:p}", this);
    }), ());
}
...
fn Class<TH: TypeHolderTrait<TH>>() -> DOMJSClass { DOMJSClass {
    ...
    malloc_size_of: malloc_size_of_including_raw_self::<ImageData<TH>> as unsafe fn(&mut _, _) -> _,
}
}}

#[inline]
fn malloc_size<TH: TypeHolderTrait<TH>>(ops: &mut MallocSizeOfOps, obj: *const c_void) -> usize {
    malloc_size_of_including_raw_self::<ImageData<TH>>(ops, obj)
}

pub unsafe fn Wrap<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<ImageData<TH>>) -> DomRoot<ImageData<TH>> {
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for ImageData<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait<TH>> PartialEq for ImageData<TH> {
    fn eq(&self, other: &ImageData<TH>) -> bool {
        self as *const ImageData<TH> == &*other
    }
}
...
pub trait ImageDataMethods {
    fn Width(&self) -> u32;
    fn Height(&self) -> u32;
    unsafe fn Data(&self, cx: *mut JSContext) -> NonNull<JSObject>;
}
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::ImageData as usize]);
    if !rval.get().is_null() {
        return;
    }
        ...
        match argcount {
            2 => {
                if args.get(0).get().is_object() {
                    loop {
                    let arg0: *mut JSObject = args.get(0).get().to_object();
                    ...
                    let arg2: Option<u32> = if args.get(2).is_undefined() {
                        ...
                        Some(match FromJSValConvertible::from_jsval(cx, args.get(2), ConversionBehavior::Default) {
                            Ok(ConversionResult::Success(v)) => v,
                            Ok(ConversionResult::Failure(error)) => {
                                throw_type_error(cx, &error);
                                return false;
                ...
                if args.get(0).get().is_object() && { rooted!(in(cx) let obj = args.get(0).get().to_object()); let mut is_date = false; assert!(JS_ObjectIsDate(cx, obj.handle(), &mut is_date)); is_date } {
                    let arg0: *mut JSObject = if args.get(0).get().is_object() {
                        args.get(0).get().to_object()
                    } else {
                        throw_type_error(cx, "Value is not an object.");
