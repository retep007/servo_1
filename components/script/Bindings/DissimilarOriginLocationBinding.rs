...
pub use self::DissimilarOriginLocationBinding::{Wrap, DissimilarOriginLocationMethods, GetProtoObject};
...
(cx: *mut JSContext, obj: HandleObject, this: *const DissimilarOriginLocation<TH>, args: JSJitSetterCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        let this = &*this;
...
(cx: *mut JSContext, _obj: HandleObject, this: *const DissimilarOriginLocation<TH>, args: *const JSJitMethodCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        if argc < 1 {
            throw_type_error(cx, "Not enough arguments to \"DissimilarOriginLocation.assign\".");
            return false;
        }
        let arg0: USVString = match FromJSValConvertible::from_jsval(cx, args.get(0), ()) {
            Ok(ConversionResult::Success(strval)) => strval,
...
(cx: *mut JSContext, _obj: HandleObject, this: *const DissimilarOriginLocation<TH>, args: *const JSJitMethodCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        if argc < 1 {
            throw_type_error(cx, "Not enough arguments to \"DissimilarOriginLocation.replace\".");
            return false;
        }
        let arg0: USVString = match FromJSValConvertible::from_jsval(cx, args.get(0), ()) {
            Ok(ConversionResult::Success(strval)) => strval,
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<DissimilarOriginLocation<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut DissimilarOriginLocation<TH>);
            }
            debug!("DissimilarOriginLocation<TH> finalize: {:p}", this);
    }), ());
}
...
fn Class<TH: TypeHolderTrait<TH>>() -> DOMJSClass { DOMJSClass {
    base: js::jsapi::JSClass {
        name: b"DissimilarOriginLocation\0" as *const u8 as *const libc::c_char,
...
fn malloc_size<TH: TypeHolderTrait<TH>>(ops: &mut MallocSizeOfOps, obj: *const c_void) -> usize {
    malloc_size_of_including_raw_self::<DissimilarOriginLocation<TH>>(ops, obj)
}

pub unsafe fn Wrap<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<DissimilarOriginLocation<TH>>) -> DomRoot<DissimilarOriginLocation<TH>> {
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for DissimilarOriginLocation<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait<TH>> PartialEq for DissimilarOriginLocation<TH> {
    fn eq(&self, other: &DissimilarOriginLocation<TH>) -> bool {
        self as *const DissimilarOriginLocation<TH> == &*other
    }
}

pub trait DissimilarOriginLocationMethods<TH: TypeHolderTrait<TH>> {
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::DissimilarOriginLocation as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"DissimilarOriginLocationPrototype\0" as *const u8 as *const libc::c_char,
    flags:
        // JSCLASS_HAS_RESERVED_SLOTS(1)
        (1 & JSCLASS_RESERVED_SLOTS_MASK) << JSCLASS_RESERVED_SLOTS_SHIFT,
    cOps: 0 as *const _,
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    assert!((*cache)[PrototypeList::ID::DissimilarOriginLocation as usize].is_null());
    (*cache)[PrototypeList::ID::DissimilarOriginLocation as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::DissimilarOriginLocation as isize),
                                  ptr::null_mut(),
                                  prototype.get());
...
} // mod DissimilarOriginLocationBinding
