...
pub use self::CanvasPatternBinding::{Wrap, GetProtoObject, DefineDOMInterface};
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<CanvasPattern<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut CanvasPattern<TH>);
            }
            debug!("CanvasPattern<TH> finalize: {:p}", this);
    }), ());
}
...
fn Class<TH: TypeHolderTrait<TH>>() -> DOMJSClass { DOMJSClass {
    ...
    malloc_size_of: malloc_size_of_including_raw_self::<CanvasPattern<TH>> as unsafe fn(&mut _, _) -> _,
}
}}

#[inline]
fn malloc_size<TH: TypeHolderTrait<TH>>(ops: &mut MallocSizeOfOps, obj: *const c_void) -> usize {
    malloc_size_of_including_raw_self::<CanvasPattern<TH>>(ops, obj)
}

pub unsafe fn Wrap<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<CanvasPattern<TH>>) -> DomRoot<CanvasPattern<TH>> {
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for CanvasPattern<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait<TH>> PartialEq for CanvasPattern<TH> {
    fn eq(&self, other: &CanvasPattern<TH>) -> bool {
        self as *const CanvasPattern<TH> == &*other
    }
}
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::CanvasPattern as usize]);
    if !rval.get().is_null() {
        return;
    }
