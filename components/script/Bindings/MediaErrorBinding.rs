...
pub use self::MediaErrorBinding::{MediaErrorConstants, Wrap, MediaErrorMethods, GetProtoObject, DefineDOMInterface};
...
(cx: *mut JSContext, _obj: HandleObject, this: *const MediaError<TH>, args: JSJitGetterCallArgs) -> bool {
...
fn code_getterinfo<TH: TypeHolderTrait<TH>>() -> JSJitInfo { JSJitInfo {
    ...
    protoID: PrototypeList::ID::MediaError as u16,
    depth: 0,
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<MediaError<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut MediaError<TH>);
            }
            debug!("MediaError<TH> finalize: {:p}", this);
    }), ());
}
...
fn Class<TH: TypeHolderTrait<TH>>() -> DOMJSClass { DOMJSClass {
    base: js::jsapi::JSClass {
        name: b"MediaError\0" as *const u8 as *const libc::c_char,
    ...
    dom_class: DOMClass {
    interface_chain: [ PrototypeList::ID::MediaError, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last ],
    type_id: ::dom::bindings::codegen::InheritTypes::TopTypeId { alone: () },
    global: InterfaceObjectMap::Globals::EMPTY,
    malloc_size_of: malloc_size_of_including_raw_self::<MediaError<TH>> as unsafe fn(&mut _, _) -> _,
}
}}

#[inline]
fn malloc_size<TH: TypeHolderTrait<TH>>(ops: &mut MallocSizeOfOps, obj: *const c_void) -> usize {
    malloc_size_of_including_raw_self::<MediaError<TH>>(ops, obj)
}

pub unsafe fn Wrap<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<MediaError<TH>>) -> DomRoot<MediaError<TH>> {
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for MediaError<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait<TH>> PartialEq for MediaError<TH> {
    fn eq(&self, other: &MediaError<TH>) -> bool {
        self as *const MediaError<TH> == &*other
    }
}

pub trait MediaErrorMethods {
    fn Code(&self) -> u16;
...
&[
    ConstantSpec { name: b"MEDIA_ERR_ABORTED\0", value: ConstantVal::IntVal(1) },
    ConstantSpec { name: b"MEDIA_ERR_NETWORK\0", value: ConstantVal::IntVal(2) },
    ConstantSpec { name: b"MEDIA_ERR_DECODE\0", value: ConstantVal::IntVal(3) },
    ConstantSpec { name: b"MEDIA_ERR_SRC_NOT_SUPPORTED\0", value: ConstantVal::IntVal(4) }]
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::MediaError as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"MediaErrorPrototype\0" as *const u8 as *const libc::c_char,
    flags:
...
static INTERFACE_OBJECT_CLASS: NonCallbackInterfaceObjectClass =
    NonCallbackInterfaceObjectClass::new(
        ...
        b"function MediaError() {\n    [native code]\n}",
        PrototypeList::ID::MediaError,
        0);
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    assert!((*cache)[PrototypeList::ID::MediaError as usize].is_null());
    (*cache)[PrototypeList::ID::MediaError as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::MediaError as isize),
                                  ptr::null_mut(),
                                  prototype.get());
