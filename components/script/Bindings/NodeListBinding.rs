...
pub use self::NodeListBinding::{Wrap, NodeListMethods, GetProtoObject, GetConstructorObject, DefineDOMInterface};
...
(cx: *mut JSContext, _obj: HandleObject, this: *const NodeList<TH>, args: *const JSJitMethodCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        if argc < 1 {
            throw_type_error(cx, "Not enough arguments to \"NodeList.item\".");
            return false;
        }
        ...
        let result: Option<DomRoot<Node<TH>>> = this.Item(arg0);
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<NodeList<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut NodeList<TH>);
            }
            debug!("NodeList<TH> finalize: {:p}", this);
    }), ());
}
...
(obj: RawHandleObject) -> *const NodeList<TH> {
    /*if (xpc::WrapperFactory::IsXrayWrapper(obj)) {
        obj = js::UnwrapObject(obj);
    }*/
    //MOZ_ASSERT(IsProxy(obj));
    let box_ = GetProxyPrivate(obj.get()).to_private() as *const NodeList<TH>;
    return box_;
}
...
(cx: *mut JSContext, _proxy: RawHandleObject) -> *const i8 {
    b"NodeList\0" as *const u8 as *const i8
}
...
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<NodeList<TH>>) -> DomRoot<NodeList<TH>> {
    ...
    let handler = RegisterBindings::PROXY_HANDLERS[PrototypeList::Proxies::NodeList as usize];
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for NodeList<TH> {
    #[inline]
    fn derives(class: &'static DOMClass) -> bool {
        class.interface_chain[0] == PrototypeList::ID::NodeList
    }
}

impl<TH: TypeHolderTrait<TH>> PartialEq for NodeList<TH> {
    fn eq(&self, other: &NodeList<TH>) -> bool {
        self as *const NodeList<TH> == &*other
    }
}

pub trait NodeListMethods<TH: TypeHolderTrait<TH>> {
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::NodeList as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"NodeListPrototype\0" as *const u8 as *const libc::c_char,
    flags:
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    /* Check to see whether the interface objects are already installed */
    let proto_or_iface_array = get_proto_or_iface_array(global.get());
    rval.set((*proto_or_iface_array)[PrototypeList::Constructor::NodeList as usize]);
    if !rval.get().is_null() {
        return;
    }
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    assert!((*cache)[PrototypeList::ID::NodeList as usize].is_null());
    (*cache)[PrototypeList::ID::NodeList as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::NodeList as isize),
                                  ptr::null_mut(),
                                  prototype.get());
    ...
    assert!((*cache)[PrototypeList::Constructor::NodeList as usize].is_null());
    (*cache)[PrototypeList::Constructor::NodeList as usize] = interface.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::Constructor::NodeList as isize),
                                  ptr::null_mut(),
                                  interface.get());
