...
pub use self::VRBinding::{Wrap, VRMethods, GetProtoObject};
pub mod VRBinding {
#![allow(non_camel_case_types,non_upper_case_globals,unused_imports,unused_variables,unused_assignments,unused_mut)]
use dom;
use dom::bindings;
...
unsafe extern fn getDisplays<TH: TypeHolderTrait>
(cx: *mut JSContext, _obj: HandleObject, this: *const VR<TH>, args: *const JSJitMethodCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        let this = &*this;
        let args = &*args;
        let argc = args._base.argc_;
        let result: Rc<Promise<TH>> = this.GetDisplays();
...
const getDisplays_methodinfo: JSJitInfo = JSJitInfo {
    call: getDisplays as *const os::raw::c_void,
    protoID: PrototypeList::ID::VR as u16,
    depth: 0,
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<VR<TH>, TH>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut VR<TH>);
            }
            debug!("VR<TH> finalize: {:p}", this);
    }), ());
}
...
fn Class<TH: TypeHolderTrait>() -> DOMJSClass { DOMJSClass {
    base: js::jsapi::JSClass {
        name: b"VR\0" as *const u8 as *const libc::c_char,
    ...
    dom_class: DOMClass {
    interface_chain: [ PrototypeList::ID::VR, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last ],
    type_id: ::dom::bindings::codegen::InheritTypes::TopTypeId { alone: () },
    global: InterfaceObjectMap::Globals::EMPTY,
    malloc_size_of: malloc_size_of_including_raw_self::<VR<TH>> as unsafe fn(&mut _, _) -> _,
}
}}

#[inline]
fn malloc_size<TH: TypeHolderTrait>(ops: &mut MallocSizeOfOps, obj: *const c_void) -> usize {
    malloc_size_of_including_raw_self::<VR<TH>>(ops, obj)
}

pub unsafe fn Wrap<TH: TypeHolderTrait>
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<VR<TH>>) -> DomRoot<VR<TH>> {
...
impl<TH: TypeHolderTrait> IDLInterface for VR<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait> PartialEq for VR<TH> {
    fn eq(&self, other: &VR<TH>) -> bool {
        self as *const VR<TH> == &*other
    }
}

pub trait VRMethods<TH: TypeHolderTrait> {
    fn GetDisplays(&self) -> Rc<Promise<TH>>;
}
const sMethods_specs: &'static [&'static[JSFunctionSpec]] = &[
&[
    JSFunctionSpec {
        name: b"getDisplays\0" as *const u8 as *const libc::c_char,
        call: JSNativeWrapper { op: Some(generic_method), info: &getDisplays_methodinfo as *const _ as *const JSJitInfo },
        nargs: 0,
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::VR as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"VRPrototype\0" as *const u8 as *const libc::c_char,
    flags:
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    assert!((*cache)[PrototypeList::ID::VR as usize].is_null());
    (*cache)[PrototypeList::ID::VR as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::VR as isize),
                                  ptr::null_mut(),
                                  prototype.get());
