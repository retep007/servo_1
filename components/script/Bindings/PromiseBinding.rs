...
impl<TH: TypeHolderTrait> PromiseJobCallback {
    ...
    pub fn Call_<T: DomObject<TH>, TH: TypeHolderTrait>(&self, thisObj: &T, aExceptionHandling: ExceptionHandling) -> Fallible<(), TH> {
...
impl<TH: TypeHolderTrait> AnyCallback {
    ...
    pub fn Call_<T: DomObject<TH>, TH: TypeHolderTrait>(&self, thisObj: &T, value: HandleValue, aExceptionHandling: ExceptionHandling) -> Fallible<JSVal, TH> {
        let s = CallSetup::new(self, aExceptionHandling);
        rooted!(in(s.get_context()) let mut thisObjJS = ptr::null_mut::<JSObject>());
        wrap_call_this_object(s.get_context(), thisObj, thisObjJS.handle_mut());
        if thisObjJS.is_null() {
...
pub use self::PromiseBinding::{GetProtoObject};
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::Promise as usize]);
    if !rval.get().is_null() {
        return;
    }
