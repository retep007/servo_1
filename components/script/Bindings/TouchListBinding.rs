...
pub use self::TouchListBinding::{Wrap, TouchListMethods, GetProtoObject, DefineDOMInterface};
...
(cx: *mut JSContext, _obj: HandleObject, this: *const TouchList<TH>, args: JSJitGetterCallArgs) -> bool {
...
fn length_getterinfo<TH: TypeHolderTrait<TH>>() -> JSJitInfo { JSJitInfo {
    ...
    protoID: PrototypeList::ID::TouchList as u16,
    depth: 0,
...
(cx: *mut JSContext, _obj: HandleObject, this: *const TouchList<TH>, args: *const JSJitMethodCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        let this = &*this;
        let args = &*args;
        ...
        if argc < 1 {
            throw_type_error(cx, "Not enough arguments to \"TouchList.item\".");
            return false;
        }
        ...
        let result: Option<DomRoot<Touch<TH>>> = this.Item(arg0);
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<TouchList<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut TouchList<TH>);
            }
            debug!("TouchList<TH> finalize: {:p}", this);
    }), ());
}
...
(obj: RawHandleObject) -> *const TouchList<TH> {
    /*if (xpc::WrapperFactory::IsXrayWrapper(obj)) {
        obj = js::UnwrapObject(obj);
    }*/
    //MOZ_ASSERT(IsProxy(obj));
    let box_ = GetProxyPrivate(obj.get()).to_private() as *const TouchList<TH>;
    return box_;
}

fn Class<TH: TypeHolderTrait<TH>>() -> DOMClass { DOMClass {
    interface_chain: [ PrototypeList::ID::TouchList, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last ],
    type_id: ::dom::bindings::codegen::InheritTypes::TopTypeId { alone: () },
    global: InterfaceObjectMap::Globals::EMPTY,
    malloc_size_of: malloc_size_of_including_raw_self::<TouchList<TH>> as unsafe fn(&mut _, _) -> _,
}}
...
(cx: *mut JSContext, proxy: RawHandleObject, id: RawHandleId, desc: RawMutableHandle<PropertyDescriptor>) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        if let Some(index) = index {
            ...
            let this = &*this;
            let result: Option<DomRoot<Touch<TH>>> = this.IndexedGetter(index);
...
(cx: *mut JSContext, _proxy: RawHandleObject) -> *const i8 {
    b"TouchList\0" as *const u8 as *const i8
}
...
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<TouchList<TH>>) -> DomRoot<TouchList<TH>> {
    ...
    let handler = RegisterBindings::PROXY_HANDLERS[PrototypeList::Proxies::TouchList as usize];
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for TouchList<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait<TH>> PartialEq for TouchList<TH> {
    fn eq(&self, other: &TouchList<TH>) -> bool {
        self as *const TouchList<TH> == &*other
    }
}

pub trait TouchListMethods<TH: TypeHolderTrait<TH>> {
    fn Length(&self) -> u32;
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::TouchList as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"TouchListPrototype\0" as *const u8 as *const libc::c_char,
    flags:
...
static INTERFACE_OBJECT_CLASS: NonCallbackInterfaceObjectClass =
    NonCallbackInterfaceObjectClass::new(
        ...
        b"function TouchList() {\n    [native code]\n}",
        PrototypeList::ID::TouchList,
        0);
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    assert!((*cache)[PrototypeList::ID::TouchList as usize].is_null());
    (*cache)[PrototypeList::ID::TouchList as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::TouchList as isize),
                                  ptr::null_mut(),
                                  prototype.get());
