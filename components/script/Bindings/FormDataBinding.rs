...
pub use self::FormDataBinding::{Wrap as FormDataWrap, FormDataMethods, GetProtoObject as FormDataGetProtoObject, DefineDOMInterface as FormDataDefineDOMInterface};
        ...
        match argcount {
            2 => {
                ...
                if args.get(1).get().is_object() {
                    loop {
                    let arg1: DomRoot<Blob<TH>> = match root_from_handlevalue(args.get(1)) {
                        Ok(val) => val,
                        Err(()) => {
                            break;
                        }
            ...
            3 => {
                ...
                let arg1: DomRoot<Blob<TH>> = if args.get(1).get().is_object() {
...
(cx: *mut JSContext, _obj: HandleObject, this: *const FormData<TH>, args: *const JSJitMethodCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let result = IterableIterator::new(&*this,
                                       ...
                                       super::FormDataIteratorBinding::Wrap);
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<FormData<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut FormData<TH>);
            }
            debug!("FormData<TH> finalize: {:p}", this);
    }), ());
}
...
fn malloc_size<TH: TypeHolderTrait<TH>>(ops: &mut MallocSizeOfOps, obj: *const c_void) -> usize {
    malloc_size_of_including_raw_self::<FormData<TH>>(ops, obj)
}

pub unsafe fn Wrap<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<FormData<TH>>) -> DomRoot<FormData<TH>> {
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for FormData<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait<TH>> PartialEq for FormData<TH> {
    fn eq(&self, other: &FormData<TH>) -> bool {
        self as *const FormData<TH> == &*other
    }
}

pub trait FormDataMethods<TH: TypeHolderTrait<TH>> {
    fn Append(&self, name: USVString, value: USVString) -> ();
    fn Append_(&self, name: USVString, value: &Blob<TH>, filename: Option<USVString>) -> ();
    fn Delete(&self, name: USVString) -> ();
    fn Get(&self, name: USVString) -> Option<UnionTypes::FileOrUSVString<TH>>;
    fn GetAll(&self, name: USVString) -> Vec<UnionTypes::FileOrUSVString<TH>>;
    fn Has(&self, name: USVString) -> bool;
    fn Set(&self, name: USVString, value: USVString) -> ();
    fn Set_(&self, name: USVString, value: &Blob<TH>, filename: Option<USVString>) -> ();
}
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::FormData as usize]);
    if !rval.get().is_null() {
        return;
    }
...
(cx: *mut JSContext, argc: u32, vp: *mut JSVal) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let arg0: Option<DomRoot<HTMLFormElement<TH>>> = if args.get(0).is_undefined() {
            None
        } else {
...
pub use self::FormDataIteratorBinding::{Wrap as FormDataIteratorWrap, FormDataIteratorMethods, GetProtoObject as FormDataIteratorGetProtoObject};
pub mod FormDataIteratorBinding {
#![allow(non_camel_case_types,non_upper_case_globals,unused_imports,unused_variables,unused_assignments,unused_mut)]
use dom;
use dom::bindings;
use dom::bindings::callback::CallSetup;
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<IterableIterator<FormData<TH>>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for IterableIterator<FormData<TH>> {
    #[inline]
    fn derives(class: &'static DOMClass) -> bool {
        class as *const _ == &Class::<TH>().dom_class as *const _
    }
...
impl<TH: TypeHolderTrait<TH>> PartialEq for IterableIterator<FormData<TH>> {
    fn eq(&self, other: &IterableIterator<FormData<TH>>) -> bool {
        self as *const IterableIterator<FormData<TH>> == &*other
    }
}

pub trait FormDataIteratorMethods<TH: TypeHolderTrait<TH>> {
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    /* Check to see whether the interface objects are already installed */
    let proto_or_iface_array = get_proto_or_iface_array(global.get());
    rval.set((*proto_or_iface_array)[PrototypeList::ID::FormDataIterator as usize]);
    if !rval.get().is_null() {
        return;
    }
