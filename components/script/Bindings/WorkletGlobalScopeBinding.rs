...
pub use self::WorkletGlobalScopeBinding::{Wrap, GetProtoObject, GetConstructorObject, DefineDOMInterface};
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<WorkletGlobalScope<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut WorkletGlobalScope<TH>);
            }
            debug!("WorkletGlobalScope<TH> finalize: {:p}", this);
    }), ());
}
...
fn Class<TH: TypeHolderTrait<TH>>() -> DOMJSClass { DOMJSClass {
    base: js::jsapi::JSClass {
        name: b"WorkletGlobalScope\0" as *const u8 as *const libc::c_char,
    ...
    dom_class: DOMClass {
    interface_chain: [ PrototypeList::ID::EventTarget, PrototypeList::ID::GlobalScope, PrototypeList::ID::WorkletGlobalScope, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last ],
    type_id: ::dom::bindings::codegen::InheritTypes::TopTypeId { eventtarget: (::dom::bindings::codegen::InheritTypes::EventTargetTypeId::GlobalScope(::dom::bindings::codegen::InheritTypes::GlobalScopeTypeId::WorkletGlobalScope(::dom::bindings::codegen::InheritTypes::WorkletGlobalScopeTypeId::WorkletGlobalScope))) },
    global: InterfaceObjectMap::Globals::EMPTY,
    malloc_size_of: malloc_size_of_including_raw_self::<WorkletGlobalScope<TH>> as unsafe fn(&mut _, _) -> _,
}
}}

#[inline]
fn malloc_size<TH: TypeHolderTrait<TH>>(ops: &mut MallocSizeOfOps, obj: *const c_void) -> usize {
    malloc_size_of_including_raw_self::<WorkletGlobalScope<TH>>(ops, obj)
}

pub unsafe fn Wrap<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<WorkletGlobalScope<TH>>) -> DomRoot<WorkletGlobalScope<TH>> {
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for WorkletGlobalScope<TH> {
    #[inline]
    fn derives(class: &'static DOMClass) -> bool {
        class.interface_chain[2] == PrototypeList::ID::WorkletGlobalScope
    }
}

impl<TH: TypeHolderTrait<TH>> PartialEq for WorkletGlobalScope<TH> {
    fn eq(&self, other: &WorkletGlobalScope<TH>) -> bool {
        self as *const WorkletGlobalScope<TH> == &*other
    }
}
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::WorkletGlobalScope as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"WorkletGlobalScopePrototype\0" as *const u8 as *const libc::c_char,
    flags:
...
static INTERFACE_OBJECT_CLASS: NonCallbackInterfaceObjectClass =
    NonCallbackInterfaceObjectClass::new(
        ...
        b"function WorkletGlobalScope() {\n    [native code]\n}",
        PrototypeList::ID::WorkletGlobalScope,
        2);
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    /* Check to see whether the interface objects are already installed */
    let proto_or_iface_array = get_proto_or_iface_array(global.get());
    rval.set((*proto_or_iface_array)[PrototypeList::Constructor::WorkletGlobalScope as usize]);
    if !rval.get().is_null() {
        return;
    }
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    assert!((*cache)[PrototypeList::ID::WorkletGlobalScope as usize].is_null());
    (*cache)[PrototypeList::ID::WorkletGlobalScope as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::WorkletGlobalScope as isize),
                                  ptr::null_mut(),
                                  prototype.get());
    ...
    create_noncallback_interface_object(cx,
                                        ...
                                        &[],
                                        prototype.handle(),
                                        b"WorkletGlobalScope\0",
                                        0,
                                        interface.handle_mut());
    assert!(!interface.is_null());
    assert!((*cache)[PrototypeList::Constructor::WorkletGlobalScope as usize].is_null());
    (*cache)[PrototypeList::Constructor::WorkletGlobalScope as usize] = interface.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::Constructor::WorkletGlobalScope as isize),
                                  ptr::null_mut(),
                                  interface.get());

}
} // mod WorkletGlobalScopeBinding
