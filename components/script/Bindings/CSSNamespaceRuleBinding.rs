...
pub use self::CSSNamespaceRuleBinding::{Wrap, CSSNamespaceRuleMethods, GetProtoObject, DefineDOMInterface};
...
use dom::bindings::weakref::WeakReferenceable;
use dom::globalscope::GlobalScope;
use dom::types::CSSNamespaceRule;
...
(cx: *mut JSContext, _obj: HandleObject, this: *const CSSNamespaceRule<TH>, args: JSJitGetterCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        let this = &*this;
        let result: DOMString = this.NamespaceURI();
...
(cx: *mut JSContext, _obj: HandleObject, this: *const CSSNamespaceRule<TH>, args: JSJitGetterCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        let this = &*this;
        let result: DOMString = this.Prefix();
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<CSSNamespaceRule<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut CSSNamespaceRule<TH>);
            }
            debug!("CSSNamespaceRule<TH> finalize: {:p}", this);
    }), ());
}
...
fn Class<TH: TypeHolderTrait<TH>>() -> DOMJSClass { DOMJSClass {
    base: js::jsapi::JSClass {
        name: b"CSSNamespaceRule\0" as *const u8 as *const libc::c_char,
...
fn malloc_size<TH: TypeHolderTrait<TH>>(ops: &mut MallocSizeOfOps, obj: *const c_void) -> usize {
    malloc_size_of_including_raw_self::<CSSNamespaceRule<TH>>(ops, obj)
}

pub unsafe fn Wrap<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<CSSNamespaceRule<TH>>) -> DomRoot<CSSNamespaceRule<TH>> {
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for CSSNamespaceRule<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait<TH>> PartialEq for CSSNamespaceRule<TH> {
    fn eq(&self, other: &CSSNamespaceRule<TH>) -> bool {
        self as *const CSSNamespaceRule<TH> == &*other
    }
}
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::CSSNamespaceRule as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"CSSNamespaceRulePrototype\0" as *const u8 as *const libc::c_char,
    flags:
...
static INTERFACE_OBJECT_CLASS: NonCallbackInterfaceObjectClass =
    NonCallbackInterfaceObjectClass::new(
        ...
        b"function CSSNamespaceRule() {\n    [native code]\n}",
        PrototypeList::ID::CSSNamespaceRule,
        1);
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    assert!((*cache)[PrototypeList::ID::CSSNamespaceRule as usize].is_null());
    (*cache)[PrototypeList::ID::CSSNamespaceRule as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::CSSNamespaceRule as isize),
                                  ptr::null_mut(),
                                  prototype.get());
    ...
    create_noncallback_interface_object(cx,
                                        ...
                                        &[],
                                        prototype.handle(),
                                        b"CSSNamespaceRule\0",
                                        0,
                                        interface.handle_mut());
    assert!(!interface.is_null());
}
} // mod CSSNamespaceRuleBinding
