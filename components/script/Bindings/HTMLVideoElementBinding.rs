...
pub use self::HTMLVideoElementBinding::{Wrap, GetProtoObject, GetConstructorObject, DefineDOMInterface};
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<HTMLVideoElement<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut HTMLVideoElement<TH>);
            }
            debug!("HTMLVideoElement<TH> finalize: {:p}", this);
    }), ());
}
...
fn Class<TH: TypeHolderTrait<TH>>() -> DOMJSClass { DOMJSClass {
    base: js::jsapi::JSClass {
        name: b"HTMLVideoElement\0" as *const u8 as *const libc::c_char,
    ...
    dom_class: DOMClass {
    interface_chain: [ PrototypeList::ID::EventTarget, PrototypeList::ID::Node, PrototypeList::ID::Element, PrototypeList::ID::HTMLElement, PrototypeList::ID::HTMLMediaElement, PrototypeList::ID::HTMLVideoElement ],
    type_id: ::dom::bindings::codegen::InheritTypes::TopTypeId { eventtarget: (::dom::bindings::codegen::InheritTypes::EventTargetTypeId::Node(::dom::bindings::codegen::InheritTypes::NodeTypeId::Element(::dom::bindings::codegen::InheritTypes::ElementTypeId::HTMLElement(::dom::bindings::codegen::InheritTypes::HTMLElementTypeId::HTMLMediaElement(::dom::bindings::codegen::InheritTypes::HTMLMediaElementTypeId::HTMLVideoElement))))) },
    global: InterfaceObjectMap::Globals::EMPTY,
    malloc_size_of: malloc_size_of_including_raw_self::<HTMLVideoElement<TH>> as unsafe fn(&mut _, _) -> _,
}
}}

#[inline]
fn malloc_size<TH: TypeHolderTrait<TH>>(ops: &mut MallocSizeOfOps, obj: *const c_void) -> usize {
    malloc_size_of_including_raw_self::<HTMLVideoElement<TH>>(ops, obj)
}

pub unsafe fn Wrap<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<HTMLVideoElement<TH>>) -> DomRoot<HTMLVideoElement<TH>> {
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for HTMLVideoElement<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait<TH>> PartialEq for HTMLVideoElement<TH> {
    fn eq(&self, other: &HTMLVideoElement<TH>) -> bool {
        self as *const HTMLVideoElement<TH> == &*other
    }
}
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::HTMLVideoElement as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"HTMLVideoElementPrototype\0" as *const u8 as *const libc::c_char,
    flags:
...
(cx: *mut JSContext, argc: u32, vp: *mut JSVal) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let result: Result<DomRoot<HTMLVideoElement<TH>>, Error<TH>> = html_constructor(&global, &args);
...
fn INTERFACE_OBJECT_CLASS<TH: TypeHolderTrait<TH>>() -> NonCallbackInterfaceObjectClass {
    ...
    NonCallbackInterfaceObjectClass::new(
        ...
        b"function HTMLVideoElement() {\n    [native code]\n}",
        PrototypeList::ID::HTMLVideoElement,
        5) 
}
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    /* Check to see whether the interface objects are already installed */
    let proto_or_iface_array = get_proto_or_iface_array(global.get());
    rval.set((*proto_or_iface_array)[PrototypeList::Constructor::HTMLVideoElement as usize]);
    if !rval.get().is_null() {
        return;
    }
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    HTMLMediaElementBinding::GetProtoObject(cx, global, prototype_proto.handle_mut());
    ...
    assert!((*cache)[PrototypeList::ID::HTMLVideoElement as usize].is_null());
    (*cache)[PrototypeList::ID::HTMLVideoElement as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::HTMLVideoElement as isize),
                                  ptr::null_mut(),
                                  prototype.get());


    rooted!(in(cx) let mut interface_proto = ptr::null_mut::<JSObject>());
    HTMLMediaElementBinding::GetConstructorObject(cx, global, interface_proto.handle_mut());
    ...
    create_noncallback_interface_object(cx,
                                        ...
                                        &[],
                                        prototype.handle(),
                                        b"HTMLVideoElement\0",
                                        0,
                                        interface.handle_mut());
    assert!(!interface.is_null());
    assert!((*cache)[PrototypeList::Constructor::HTMLVideoElement as usize].is_null());
    (*cache)[PrototypeList::Constructor::HTMLVideoElement as usize] = interface.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::Constructor::HTMLVideoElement as isize),
                                  ptr::null_mut(),
                                  interface.get());

}
} // mod HTMLVideoElementBinding
