...
pub use self::CSSMediaRuleBinding::{Wrap, CSSMediaRuleMethods, GetProtoObject, DefineDOMInterface};
...
use dom::bindings::callback::ExceptionHandling;
use dom::bindings::callback::wrap_call_this_object;
use dom::bindings::codegen::Bindings::CSSConditionRuleBinding::CSSConditionRuleBinding;
use dom::bindings::codegen::InterfaceObjectMap;
use dom::bindings::codegen::PrototypeList;
use dom::bindings::codegen::RegisterBindings;
use dom::bindings::codegen::UnionTypes;
...
use dom::bindings::weakref::WeakReferenceable;
use dom::globalscope::GlobalScope;
use dom::types::CSSConditionRule;
use dom::types::CSSMediaRule;
...
(cx: *mut JSContext, _obj: HandleObject, this: *const CSSMediaRule<TH>, args: JSJitGetterCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        let this = &*this;
        let result: DomRoot<MediaList<TH>> = this.Media();
...
(cx: *mut JSContext, obj: HandleObject, this: *const CSSMediaRule<TH>, args: JSJitSetterCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        rooted!(in(cx) let mut v = UndefinedValue());
        if !JS_GetProperty(cx, obj, b"media\0" as *const u8 as *const libc::c_char, v.handle_mut()) {
            return false;
        }
        if !v.is_object() {
            throw_type_error(cx, "Value.media is not an object.");
            return false;
        }
        rooted!(in(cx) let target_obj = v.to_object());
        JS_SetProperty(cx, target_obj.handle(), b"mediaText\0" as *const u8 as *const libc::c_char, args.get(0))
...
fn media_getterinfo<TH: TypeHolderTrait<TH>>() -> JSJitInfo { JSJitInfo {
    ...
    protoID: PrototypeList::ID::CSSMediaRule as u16,
    depth: 3,
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<CSSMediaRule<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut CSSMediaRule<TH>);
            }
            debug!("CSSMediaRule<TH> finalize: {:p}", this);
    }), ());
}
...
fn Class<TH: TypeHolderTrait<TH>>() -> DOMJSClass { DOMJSClass {
    base: js::jsapi::JSClass {
        name: b"CSSMediaRule\0" as *const u8 as *const libc::c_char,
...
fn malloc_size<TH: TypeHolderTrait<TH>>(ops: &mut MallocSizeOfOps, obj: *const c_void) -> usize {
    malloc_size_of_including_raw_self::<CSSMediaRule<TH>>(ops, obj)
}

pub unsafe fn Wrap<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<CSSMediaRule<TH>>) -> DomRoot<CSSMediaRule<TH>> {
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for CSSMediaRule<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait<TH>> PartialEq for CSSMediaRule<TH> {
    fn eq(&self, other: &CSSMediaRule<TH>) -> bool {
        self as *const CSSMediaRule<TH> == &*other
    }
}

pub trait CSSMediaRuleMethods<TH: TypeHolderTrait<TH>> {
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::CSSMediaRule as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"CSSMediaRulePrototype\0" as *const u8 as *const libc::c_char,
    flags:
...
static INTERFACE_OBJECT_CLASS: NonCallbackInterfaceObjectClass =
    NonCallbackInterfaceObjectClass::new(
        ...
        b"function CSSMediaRule() {\n    [native code]\n}",
        PrototypeList::ID::CSSMediaRule,
        3);
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    CSSConditionRuleBinding::GetProtoObject(cx, global, prototype_proto.handle_mut());
    ...
    assert!((*cache)[PrototypeList::ID::CSSMediaRule as usize].is_null());
    (*cache)[PrototypeList::ID::CSSMediaRule as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::CSSMediaRule as isize),
                                  ptr::null_mut(),
                                  prototype.get());


    rooted!(in(cx) let mut interface_proto = ptr::null_mut::<JSObject>());
    CSSConditionRuleBinding::GetConstructorObject(cx, global, interface_proto.handle_mut());
