...
pub use self::HTMLOutputElementBinding::{Wrap, HTMLOutputElementMethods, GetProtoObject, GetConstructorObject, DefineDOMInterface};
...
use dom::types::HTMLOutputElement;
...
(cx: *mut JSContext, _obj: HandleObject, this: *const HTMLOutputElement<TH>, args: JSJitGetterCallArgs) -> bool {
...
fn form_getterinfo<TH: TypeHolderTrait<TH>>() -> JSJitInfo { JSJitInfo {
    ...
    protoID: PrototypeList::ID::HTMLOutputElement as u16,
    depth: 4,
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<HTMLOutputElement<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut HTMLOutputElement<TH>);
            }
            debug!("HTMLOutputElement<TH> finalize: {:p}", this);
    }), ());
}
...
fn Class<TH: TypeHolderTrait<TH>>() -> DOMJSClass { DOMJSClass {
    base: js::jsapi::JSClass {
        name: b"HTMLOutputElement\0" as *const u8 as *const libc::c_char,
    ...
    dom_class: DOMClass {
    interface_chain: [ PrototypeList::ID::EventTarget, PrototypeList::ID::Node, PrototypeList::ID::Element, PrototypeList::ID::HTMLElement, PrototypeList::ID::HTMLOutputElement, PrototypeList::ID::Last ],
    ...
    malloc_size_of: malloc_size_of_including_raw_self::<HTMLOutputElement<TH>> as unsafe fn(&mut _, _) -> _,
}
}}

#[inline]
fn malloc_size<TH: TypeHolderTrait<TH>>(ops: &mut MallocSizeOfOps, obj: *const c_void) -> usize {
    malloc_size_of_including_raw_self::<HTMLOutputElement<TH>>(ops, obj)
}

pub unsafe fn Wrap<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<HTMLOutputElement<TH>>) -> DomRoot<HTMLOutputElement<TH>> {
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for HTMLOutputElement<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait<TH>> PartialEq for HTMLOutputElement<TH> {
    fn eq(&self, other: &HTMLOutputElement<TH>) -> bool {
        self as *const HTMLOutputElement<TH> == &*other
    }
}

pub trait HTMLOutputElementMethods<TH: TypeHolderTrait<TH>> {
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::HTMLOutputElement as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"HTMLOutputElementPrototype\0" as *const u8 as *const libc::c_char,
    flags:
...
(cx: *mut JSContext, argc: u32, vp: *mut JSVal) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let result: Result<DomRoot<HTMLOutputElement<TH>>, Error<TH>> = html_constructor(&global, &args);
...
fn INTERFACE_OBJECT_CLASS<TH: TypeHolderTrait<TH>>() -> NonCallbackInterfaceObjectClass {
    ...
    NonCallbackInterfaceObjectClass::new(
        ...
        b"function HTMLOutputElement() {\n    [native code]\n}",
        PrototypeList::ID::HTMLOutputElement,
        4) 
}
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    /* Check to see whether the interface objects are already installed */
    let proto_or_iface_array = get_proto_or_iface_array(global.get());
    rval.set((*proto_or_iface_array)[PrototypeList::Constructor::HTMLOutputElement as usize]);
    if !rval.get().is_null() {
        return;
    }
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    assert!((*cache)[PrototypeList::ID::HTMLOutputElement as usize].is_null());
    (*cache)[PrototypeList::ID::HTMLOutputElement as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::HTMLOutputElement as isize),
                                  ptr::null_mut(),
                                  prototype.get());
    ...
    create_noncallback_interface_object(cx,
                                        ...
                                        &[],
                                        prototype.handle(),
                                        b"HTMLOutputElement\0",
                                        0,
                                        interface.handle_mut());
    assert!(!interface.is_null());
    assert!((*cache)[PrototypeList::Constructor::HTMLOutputElement as usize].is_null());
    (*cache)[PrototypeList::Constructor::HTMLOutputElement as usize] = interface.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::Constructor::HTMLOutputElement as isize),
                                  ptr::null_mut(),
                                  interface.get());

}
} // mod HTMLOutputElementBinding
