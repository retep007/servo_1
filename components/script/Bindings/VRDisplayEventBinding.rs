...
pub mod VRDisplayEventReasonValues {
    use dom::bindings::conversions::ToJSValConvertible;
    use js::jsapi::JSContext;
    use js::rust::MutableHandleValue;
    use js::jsval::JSVal;

    pub const pairs: &'static [(&'static str, super::VRDisplayEventReason)] = &[
        ("navigation", super::VRDisplayEventReason::Navigation),
        ("mounted", super::VRDisplayEventReason::Mounted),
        ("unmounted", super::VRDisplayEventReason::Unmounted),
        ("requested", super::VRDisplayEventReason::Requested),
    ];

    impl super::VRDisplayEventReason {
        pub fn as_str(&self) -> &'static str {
            pairs[*self as usize].0
        }
    }

    impl Default for super::VRDisplayEventReason {
        fn default() -> super::VRDisplayEventReason {
            pairs[0].1
        }
    }

    impl ToJSValConvertible for super::VRDisplayEventReason {
...
#[derive(JSTraceable)]
pub struct VRDisplayEventInit<TH: TypeHolderTrait<TH> + 'static> {
...
impl<TH: TypeHolderTrait<TH>> VRDisplayEventInit<TH> {
    ...
    pub unsafe fn new(cx: *mut JSContext, val: HandleValue)
        ...
        rooted!(in(cx) let object = object);
            let dictionary = VRDisplayEventInit {
            parent: {
            ...
            display: {
                ...
                match try!(get_dictionary_property(cx, object.handle(), "display", rval.handle_mut())) {
                    true => {
                        if rval.handle().get().is_object() {
                            match root_from_handlevalue(rval.handle()) {
                                Ok(val) => val,
                                Err(()) => {
                                    throw_type_error(cx, "value does not implement interface VRDisplay.");
                                    return Err(());
                                }
                            }
                    ...
                    false => {
                        throw_type_error(cx, "Missing required member \"display\".");
                        return Err(());
                    },
                }
            },
            reason: {
                rooted!(in(cx) let mut rval = UndefinedValue());
                match try!(get_dictionary_property(cx, object.handle(), "reason", rval.handle_mut())) {
                    true => {
                        Some(match find_enum_value(cx, rval.handle(), VRDisplayEventReasonValues::pairs) {
                            Err(_) => { return Err(()); },
                            Ok((None, search)) => { throw_type_error(cx, &format!("'{}' is not a valid enum value for enumeration 'VRDisplayEventReason'.", search)); return Err(()); },
...
impl<TH: TypeHolderTrait<TH>> FromJSValConvertible for VRDisplayEventInit<TH> {
    type Config = ();
    unsafe fn from_jsval(cx: *mut JSContext, value: HandleValue, _option: ())
                         -> Result<ConversionResult<VRDisplayEventInit<TH>>, ()> {
        VRDisplayEventInit::new(cx, value)
    }
}

impl<TH: TypeHolderTrait<TH>> ToJSValConvertible for VRDisplayEventInit<TH> {
    unsafe fn to_jsval(&self, cx: *mut JSContext, mut rval: MutableHandleValue) {
        rooted!(in(cx) let obj = JS_NewObject(cx, ptr::null()));
        let display = &self.display;
        rooted!(in(cx) let mut display_js = UndefinedValue());
        display.to_jsval(cx, display_js.handle_mut());
        set_dictionary_property(cx, obj.handle(), "display", display_js.handle()).unwrap();
        if let Some(ref reason) = self.reason {
            rooted!(in(cx) let mut reason_js = UndefinedValue());
            reason.to_jsval(cx, reason_js.handle_mut());
            set_dictionary_property(cx, obj.handle(), "reason", reason_js.handle()).unwrap();
        }
...
pub use self::VRDisplayEventBinding::{Wrap, VRDisplayEventMethods, GetProtoObject, DefineDOMInterface};
...
use dom::bindings::codegen::Bindings::VRDisplayEventBinding::VRDisplayEventReasonValues;
use dom::bindings::codegen::InterfaceObjectMap;
use dom::bindings::codegen::PrototypeList;
use dom::bindings::codegen::RegisterBindings;
use dom::bindings::codegen::UnionTypes;
...
(cx: *mut JSContext, _obj: HandleObject, this: *const VRDisplayEvent<TH>, args: JSJitGetterCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        let this = &*this;
        let result: DomRoot<VRDisplay<TH>> = this.Display();
...
fn display_getterinfo<TH: TypeHolderTrait<TH>>() -> JSJitInfo { JSJitInfo {
    ...
    protoID: PrototypeList::ID::VRDisplayEvent as u16,
    depth: 1,
...
unsafe extern fn get_reason<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, _obj: HandleObject, this: *const VRDisplayEvent<TH>, args: JSJitGetterCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        let this = &*this;
        let result: Option<VRDisplayEventReason> = this.GetReason();
...
fn reason_getterinfo<TH: TypeHolderTrait<TH>>() -> JSJitInfo { JSJitInfo {
    call: get_reason::<TH> as *const os::raw::c_void,
    protoID: PrototypeList::ID::VRDisplayEvent as u16,
    depth: 1,
    _bitfield_1: new_jsjitinfo_bitfield_1!(
        JSJitInfo_OpType::Getter as u8,
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<VRDisplayEvent<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut VRDisplayEvent<TH>);
            }
            debug!("VRDisplayEvent<TH> finalize: {:p}", this);
    }), ());
}
...
fn Class<TH: TypeHolderTrait<TH>>() -> DOMJSClass { DOMJSClass {
    base: js::jsapi::JSClass {
        name: b"VRDisplayEvent\0" as *const u8 as *const libc::c_char,
    ...
    dom_class: DOMClass {
    interface_chain: [ PrototypeList::ID::Event, PrototypeList::ID::VRDisplayEvent, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last ],
    type_id: ::dom::bindings::codegen::InheritTypes::TopTypeId { event: (::dom::bindings::codegen::InheritTypes::EventTypeId::VRDisplayEvent) },
    global: InterfaceObjectMap::Globals::EMPTY,
    malloc_size_of: malloc_size_of_including_raw_self::<VRDisplayEvent<TH>> as unsafe fn(&mut _, _) -> _,
}
}}

#[inline]
fn malloc_size<TH: TypeHolderTrait<TH>>(ops: &mut MallocSizeOfOps, obj: *const c_void) -> usize {
    malloc_size_of_including_raw_self::<VRDisplayEvent<TH>>(ops, obj)
}

pub unsafe fn Wrap<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<VRDisplayEvent<TH>>) -> DomRoot<VRDisplayEvent<TH>> {
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for VRDisplayEvent<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait<TH>> PartialEq for VRDisplayEvent<TH> {
    fn eq(&self, other: &VRDisplayEvent<TH>) -> bool {
        self as *const VRDisplayEvent<TH> == &*other
    }
}

pub trait VRDisplayEventMethods<TH: TypeHolderTrait<TH>> {
...
&[
    ...
    JSPropertySpec {
        name: b"reason\0" as *const u8 as *const libc::c_char,
        flags: (JSPROP_ENUMERATE | JSPROP_SHARED) as u8,
        getter: JSNativeWrapper { op: Some(generic_getter), info: &reason_getterinfo::<TH>() },
        setter: JSNativeWrapper { op: None, info: 0 as *const JSJitInfo }
    },
    JSPropertySpec {
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::VRDisplayEvent as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"VRDisplayEventPrototype\0" as *const u8 as *const libc::c_char,
    flags:
        // JSCLASS_HAS_RESERVED_SLOTS(1)
        (1 & JSCLASS_RESERVED_SLOTS_MASK) << JSCLASS_RESERVED_SLOTS_SHIFT,
    cOps: 0 as *const _,
...
(cx: *mut JSContext, argc: u32, vp: *mut JSVal) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        if argc < 2 {
            throw_type_error(cx, "Not enough arguments to \"VRDisplayEvent.constructor\".");
            return false;
        }
...
fn INTERFACE_OBJECT_CLASS<TH: TypeHolderTrait<TH>>() -> NonCallbackInterfaceObjectClass {
    ...
    NonCallbackInterfaceObjectClass::new(
        ...
        b"function VRDisplayEvent() {\n    [native code]\n}",
        PrototypeList::ID::VRDisplayEvent,
        1) 
}
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    assert!((*cache)[PrototypeList::ID::VRDisplayEvent as usize].is_null());
    (*cache)[PrototypeList::ID::VRDisplayEvent as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::VRDisplayEvent as isize),
                                  ptr::null_mut(),
                                  prototype.get());
