...
pub use self::HTMLCollectionBinding::{Wrap, HTMLCollectionMethods, GetProtoObject, GetConstructorObject, DefineDOMInterface};
...
(cx: *mut JSContext, _obj: HandleObject, this: *const HTMLCollection<TH>, args: JSJitGetterCallArgs) -> bool {
...
(cx: *mut JSContext, _obj: HandleObject, this: *const HTMLCollection<TH>, args: *const JSJitMethodCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        if argc < 1 {
            throw_type_error(cx, "Not enough arguments to \"HTMLCollection.item\".");
            return false;
        }
...
(cx: *mut JSContext, _obj: HandleObject, this: *const HTMLCollection<TH>, args: *const JSJitMethodCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        if argc < 1 {
            throw_type_error(cx, "Not enough arguments to \"HTMLCollection.namedItem\".");
            return false;
        }
        ...
        let result: Option<DomRoot<Element<TH>>> = this.NamedItem(arg0);
...
fn namedItem_methodinfo<TH: TypeHolderTrait<TH>>() -> JSTypedMethodJitInfo { JSTypedMethodJitInfo {
    base:   JSJitInfo {
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<HTMLCollection<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut HTMLCollection<TH>);
            }
            debug!("HTMLCollection<TH> finalize: {:p}", this);
    }), ());
}
...
(obj: RawHandleObject) -> *const HTMLCollection<TH> {
    /*if (xpc::WrapperFactory::IsXrayWrapper(obj)) {
        obj = js::UnwrapObject(obj);
    }*/
    //MOZ_ASSERT(IsProxy(obj));
    let box_ = GetProxyPrivate(obj.get()).to_private() as *const HTMLCollection<TH>;
    return box_;
}

fn Class<TH: TypeHolderTrait<TH>>() -> DOMClass { DOMClass {
    interface_chain: [ PrototypeList::ID::HTMLCollection, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last ],
    type_id: ::dom::bindings::codegen::InheritTypes::TopTypeId { htmlcollection: (::dom::bindings::codegen::InheritTypes::HTMLCollectionTypeId::HTMLCollection) },
    global: InterfaceObjectMap::Globals::EMPTY,
    malloc_size_of: malloc_size_of_including_raw_self::<HTMLCollection<TH>> as unsafe fn(&mut _, _) -> _,
}}
...
(cx: *mut JSContext, _proxy: RawHandleObject) -> *const i8 {
    b"HTMLCollection\0" as *const u8 as *const i8
}
...
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<HTMLCollection<TH>>) -> DomRoot<HTMLCollection<TH>> {
    ...
    let handler = RegisterBindings::PROXY_HANDLERS[PrototypeList::Proxies::HTMLCollection as usize];
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for HTMLCollection<TH> {
    #[inline]
    fn derives(class: &'static DOMClass) -> bool {
        class.interface_chain[0] == PrototypeList::ID::HTMLCollection
    }
}

impl<TH: TypeHolderTrait<TH>> PartialEq for HTMLCollection<TH> {
    fn eq(&self, other: &HTMLCollection<TH>) -> bool {
        self as *const HTMLCollection<TH> == &*other
    }
}

pub trait HTMLCollectionMethods<TH: TypeHolderTrait<TH>> {
    fn Length(&self) -> u32;
    fn Item(&self, index: u32) -> Option<DomRoot<Element<TH>>>;
    fn NamedItem(&self, name: DOMString) -> Option<DomRoot<Element<TH>>>;
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::HTMLCollection as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"HTMLCollectionPrototype\0" as *const u8 as *const libc::c_char,
    flags:
...
static INTERFACE_OBJECT_CLASS: NonCallbackInterfaceObjectClass =
    NonCallbackInterfaceObjectClass::new(
        ...
        b"function HTMLCollection() {\n    [native code]\n}",
        PrototypeList::ID::HTMLCollection,
        0);
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    /* Check to see whether the interface objects are already installed */
    let proto_or_iface_array = get_proto_or_iface_array(global.get());
    rval.set((*proto_or_iface_array)[PrototypeList::Constructor::HTMLCollection as usize]);
    if !rval.get().is_null() {
        return;
    }
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    assert!((*cache)[PrototypeList::ID::HTMLCollection as usize].is_null());
    (*cache)[PrototypeList::ID::HTMLCollection as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::HTMLCollection as isize),
                                  ptr::null_mut(),
                                  prototype.get());
    ...
    assert!((*cache)[PrototypeList::Constructor::HTMLCollection as usize].is_null());
    (*cache)[PrototypeList::Constructor::HTMLCollection as usize] = interface.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::Constructor::HTMLCollection as isize),
                                  ptr::null_mut(),
                                  interface.get());

}
} // mod HTMLCollectionBinding
