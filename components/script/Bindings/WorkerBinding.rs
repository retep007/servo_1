...
pub use self::WorkerBinding::{Wrap, WorkerMethods, GetProtoObject, DefineDOMInterface};
...
unsafe extern fn terminate<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, _obj: HandleObject, this: *const Worker<TH>, args: *const JSJitMethodCallArgs) -> bool {
...
fn terminate_methodinfo<TH: TypeHolderTrait<TH>>() -> JSJitInfo { JSJitInfo {
    call: terminate::<TH> as *const os::raw::c_void,
    protoID: PrototypeList::ID::Worker as u16,
    depth: 1,
...
(cx: *mut JSContext, _obj: HandleObject, this: *const Worker<TH>, args: *const JSJitMethodCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        if argc < 1 {
            throw_type_error(cx, "Not enough arguments to \"Worker.postMessage\".");
            return false;
        }
...
(cx: *mut JSContext, _obj: HandleObject, this: *const Worker<TH>, args: JSJitGetterCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        let this = &*this;
...
(cx: *mut JSContext, obj: HandleObject, this: *const Worker<TH>, args: JSJitSetterCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        let this = &*this;
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<Worker<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut Worker<TH>);
            }
            debug!("Worker<TH> finalize: {:p}", this);
    }), ());
}
...
fn Class<TH: TypeHolderTrait<TH>>() -> DOMJSClass { DOMJSClass {
    base: js::jsapi::JSClass {
        name: b"Worker\0" as *const u8 as *const libc::c_char,
    ...
    dom_class: DOMClass {
    interface_chain: [ PrototypeList::ID::EventTarget, PrototypeList::ID::Worker, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last ],
    type_id: ::dom::bindings::codegen::InheritTypes::TopTypeId { eventtarget: (::dom::bindings::codegen::InheritTypes::EventTargetTypeId::Worker) },
    global: InterfaceObjectMap::Globals::EMPTY,
    malloc_size_of: malloc_size_of_including_raw_self::<Worker<TH>> as unsafe fn(&mut _, _) -> _,
}
}}

#[inline]
fn malloc_size<TH: TypeHolderTrait<TH>>(ops: &mut MallocSizeOfOps, obj: *const c_void) -> usize {
    malloc_size_of_including_raw_self::<Worker<TH>>(ops, obj)
}

pub unsafe fn Wrap<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<Worker<TH>>) -> DomRoot<Worker<TH>> {
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for Worker<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait<TH>> PartialEq for Worker<TH> {
    fn eq(&self, other: &Worker<TH>) -> bool {
        self as *const Worker<TH> == &*other
    }
}

pub trait WorkerMethods<TH: TypeHolderTrait<TH>> {
    fn Terminate(&self) -> ();
...
&[
    JSFunctionSpec {
        name: b"terminate\0" as *const u8 as *const libc::c_char,
        call: JSNativeWrapper { op: Some(generic_method), info: &terminate_methodinfo::<TH>() as *const _ as *const JSJitInfo },
        nargs: 0,
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::Worker as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"WorkerPrototype\0" as *const u8 as *const libc::c_char,
    flags:
...
(cx: *mut JSContext, argc: u32, vp: *mut JSVal) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        if argc < 1 {
            throw_type_error(cx, "Not enough arguments to \"Worker.constructor\".");
            return false;
        }
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    assert!((*cache)[PrototypeList::ID::Worker as usize].is_null());
    (*cache)[PrototypeList::ID::Worker as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::Worker as isize),
                                  ptr::null_mut(),
                                  prototype.get());
