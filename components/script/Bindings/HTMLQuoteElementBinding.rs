...
pub use self::HTMLQuoteElementBinding::{Wrap, GetProtoObject, GetConstructorObject, DefineDOMInterface};
...
use dom::types::HTMLQuoteElement;
use dom::windowproxy::WindowProxy;
use js;
use js::JSCLASS_GLOBAL_SLOT_COUNT;
use js::JSCLASS_IS_DOMJSCLASS;
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<HTMLQuoteElement<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut HTMLQuoteElement<TH>);
            }
            debug!("HTMLQuoteElement<TH> finalize: {:p}", this);
    }), ());
}
...
static Class: DOMJSClass = DOMJSClass {
    base: js::jsapi::JSClass {
        name: b"HTMLQuoteElement\0" as *const u8 as *const libc::c_char,
...
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<HTMLQuoteElement<TH>>) -> DomRoot<HTMLQuoteElement<TH>> {
...
impl<TH: TypeHolderTrait> IDLInterface for HTMLQuoteElement<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait> PartialEq for HTMLQuoteElement<TH> {
    fn eq(&self, other: &HTMLQuoteElement<TH>) -> bool {
        self as *const HTMLQuoteElement<TH> == &*other
    }
}
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::HTMLQuoteElement as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"HTMLQuoteElementPrototype\0" as *const u8 as *const libc::c_char,
    flags:
...
(cx: *mut JSContext, argc: u32, vp: *mut JSVal) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let result: Result<DomRoot<HTMLQuoteElement<TH>>, Error<TH>> = html_constructor(&global, &args);
...
static INTERFACE_OBJECT_CLASS: NonCallbackInterfaceObjectClass =
    ...
    NonCallbackInterfaceObjectClass::new(
        ...
        b"function HTMLQuoteElement() {\n    [native code]\n}",
        PrototypeList::ID::HTMLQuoteElement,
        4);
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    /* Check to see whether the interface objects are already installed */
    let proto_or_iface_array = get_proto_or_iface_array(global.get());
    rval.set((*proto_or_iface_array)[PrototypeList::Constructor::HTMLQuoteElement as usize]);
    if !rval.get().is_null() {
        return;
    }
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    assert!((*cache)[PrototypeList::ID::HTMLQuoteElement as usize].is_null());
    (*cache)[PrototypeList::ID::HTMLQuoteElement as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::HTMLQuoteElement as isize),
                                  ptr::null_mut(),
                                  prototype.get());
    ...
    create_noncallback_interface_object(cx,
                                        ...
                                        &[],
                                        prototype.handle(),
                                        b"HTMLQuoteElement\0",
                                        0,
                                        interface.handle_mut());
    assert!(!interface.is_null());
    assert!((*cache)[PrototypeList::Constructor::HTMLQuoteElement as usize].is_null());
    (*cache)[PrototypeList::Constructor::HTMLQuoteElement as usize] = interface.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::Constructor::HTMLQuoteElement as isize),
                                  ptr::null_mut(),
                                  interface.get());

}
} // mod HTMLQuoteElementBinding
