...
impl BluetoothDataFilterInit {
    ...
    pub unsafe fn new(cx: *mut JSContext, val: HandleValue)
        ...
        rooted!(in(cx) let object = object);
            let mut dictionary = RootedTraceableBox::new(BluetoothDataFilterInit::default());
            dictionary.dataPrefix = {
                rooted!(in(cx) let mut rval = UndefinedValue());
                match try!(get_dictionary_property(cx, object.handle(), "dataPrefix", rval.handle_mut())) {
                    true => {
            ...
            dictionary.mask = {
                rooted!(in(cx) let mut rval = UndefinedValue());
                match try!(get_dictionary_property(cx, object.handle(), "mask", rval.handle_mut())) {
                    true => {
                        Some(match FromJSValConvertible::from_jsval(cx, rval.handle(), ()) {
                            Ok(ConversionResult::Success(value)) => value,
                            Ok(ConversionResult::Failure(error)) => {
...
impl FromJSValConvertible for RootedTraceableBox<BluetoothDataFilterInit> {
    ...
    unsafe fn from_jsval(cx: *mut JSContext, value: HandleValue, _option: ())
                         -> Result<ConversionResult<RootedTraceableBox<BluetoothDataFilterInit>>, ()> {
        BluetoothDataFilterInit::new(cx, value)
    }
}

impl ToJSValConvertible for RootedTraceableBox<BluetoothDataFilterInit> {
    unsafe fn to_jsval(&self, cx: *mut JSContext, mut rval: MutableHandleValue) {
        rooted!(in(cx) let obj = JS_NewObject(cx, ptr::null()));
        if let Some(ref dataPrefix) = self.dataPrefix {
            rooted!(in(cx) let mut dataPrefix_js = UndefinedValue());
            dataPrefix.to_jsval(cx, dataPrefix_js.handle_mut());
            set_dictionary_property(cx, obj.handle(), "dataPrefix", dataPrefix_js.handle()).unwrap();
        }
...
pub struct BluetoothLEScanFilterInit {
    pub manufacturerData: Option<MozMap<RootedTraceableBox<dom::bindings::codegen::Bindings::BluetoothBinding::BluetoothDataFilterInit>>>,
    pub name: Option<DOMString>,
    pub namePrefix: Option<DOMString>,
    pub serviceData: Option<MozMap<RootedTraceableBox<dom::bindings::codegen::Bindings::BluetoothBinding::BluetoothDataFilterInit>>>,
    pub services: Option<Vec<UnionTypes::StringOrUnsignedLong>>,
}
impl BluetoothLEScanFilterInit {
    pub unsafe fn empty(cx: *mut JSContext) -> BluetoothLEScanFilterInit {
        match BluetoothLEScanFilterInit::new(cx, HandleValue::null()) {
            Ok(ConversionResult::Success(v)) => v,
            _ => unreachable!(),
        }
    }
    pub unsafe fn new(cx: *mut JSContext, val: HandleValue)
                      -> Result<ConversionResult<BluetoothLEScanFilterInit>, ()> {
        let object = if val.get().is_null_or_undefined() {
            ptr::null_mut()
        } else if val.get().is_object() {
            val.get().to_object()
        ...
        rooted!(in(cx) let object = object);
            let dictionary = BluetoothLEScanFilterInit {
            manufacturerData: {
                rooted!(in(cx) let mut rval = UndefinedValue());
                match try!(get_dictionary_property(cx, object.handle(), "manufacturerData", rval.handle_mut())) {
                    true => {
                        Some(match FromJSValConvertible::from_jsval(cx, rval.handle(), ()) {
                            Ok(ConversionResult::Success(value)) => value,
                            Ok(ConversionResult::Failure(error)) => {
            ...
            namePrefix: {
                rooted!(in(cx) let mut rval = UndefinedValue());
                match try!(get_dictionary_property(cx, object.handle(), "namePrefix", rval.handle_mut())) {
                    true => {
                        Some(match FromJSValConvertible::from_jsval(cx, rval.handle(), StringificationBehavior::Default) {
                            Ok(ConversionResult::Success(strval)) => strval,
                            Ok(ConversionResult::Failure(error)) => {
            ...
            serviceData: {
                rooted!(in(cx) let mut rval = UndefinedValue());
                match try!(get_dictionary_property(cx, object.handle(), "serviceData", rval.handle_mut())) {
                    true => {
                        Some(match FromJSValConvertible::from_jsval(cx, rval.handle(), ()) {
                            Ok(ConversionResult::Success(value)) => value,
                            Ok(ConversionResult::Failure(error)) => {
            ...
            services: {
                rooted!(in(cx) let mut rval = UndefinedValue());
                match try!(get_dictionary_property(cx, object.handle(), "services", rval.handle_mut())) {
                    true => {
                        Some(match FromJSValConvertible::from_jsval(cx, rval.handle(), ()) {
                            Ok(ConversionResult::Success(value)) => value,
                            Ok(ConversionResult::Failure(error)) => {
...
impl FromJSValConvertible for BluetoothLEScanFilterInit {
    ...
    unsafe fn from_jsval(cx: *mut JSContext, value: HandleValue, _option: ())
                         -> Result<ConversionResult<BluetoothLEScanFilterInit>, ()> {
        BluetoothLEScanFilterInit::new(cx, value)
    }
}
...
impl ToJSValConvertible for BluetoothLEScanFilterInit {
    unsafe fn to_jsval(&self, cx: *mut JSContext, mut rval: MutableHandleValue) {
        rooted!(in(cx) let obj = JS_NewObject(cx, ptr::null()));
        if let Some(ref manufacturerData) = self.manufacturerData {
            rooted!(in(cx) let mut manufacturerData_js = UndefinedValue());
            manufacturerData.to_jsval(cx, manufacturerData_js.handle_mut());
            set_dictionary_property(cx, obj.handle(), "manufacturerData", manufacturerData_js.handle()).unwrap();
        }
        ...
        if let Some(ref namePrefix) = self.namePrefix {
            rooted!(in(cx) let mut namePrefix_js = UndefinedValue());
            namePrefix.to_jsval(cx, namePrefix_js.handle_mut());
            set_dictionary_property(cx, obj.handle(), "namePrefix", namePrefix_js.handle()).unwrap();
        }
        if let Some(ref serviceData) = self.serviceData {
            rooted!(in(cx) let mut serviceData_js = UndefinedValue());
            serviceData.to_jsval(cx, serviceData_js.handle_mut());
            set_dictionary_property(cx, obj.handle(), "serviceData", serviceData_js.handle()).unwrap();
        }
        if let Some(ref services) = self.services {
            rooted!(in(cx) let mut services_js = UndefinedValue());
            services.to_jsval(cx, services_js.handle_mut());
            set_dictionary_property(cx, obj.handle(), "services", services_js.handle()).unwrap();
        }
        rval.set(ObjectOrNullValue(obj.get()))
    }
}
...
impl RequestDeviceOptions {
    pub unsafe fn empty(cx: *mut JSContext) -> RequestDeviceOptions {
        match RequestDeviceOptions::new(cx, HandleValue::null()) {
            Ok(ConversionResult::Success(v)) => v,
            _ => unreachable!(),
        }
    }
    pub unsafe fn new(cx: *mut JSContext, val: HandleValue)
                      -> Result<ConversionResult<RequestDeviceOptions>, ()> {
        let object = if val.get().is_null_or_undefined() {
            ptr::null_mut()
        } else if val.get().is_object() {
            val.get().to_object()
        ...
        rooted!(in(cx) let object = object);
            let dictionary = RequestDeviceOptions {
            acceptAllDevices: {
                rooted!(in(cx) let mut rval = UndefinedValue());
...
impl FromJSValConvertible for RequestDeviceOptions {
    ...
    unsafe fn from_jsval(cx: *mut JSContext, value: HandleValue, _option: ())
                         -> Result<ConversionResult<RequestDeviceOptions>, ()> {
        RequestDeviceOptions::new(cx, value)
    }
}
...
pub use self::BluetoothBinding::{Wrap, BluetoothMethods, GetProtoObject, DefineDOMInterface};
...
fn getAvailability_methodinfo<TH: TypeHolderTrait<TH>>() -> JSJitInfo {
    JSJitInfo {
...
unsafe extern fn get_onavailabilitychanged<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, _obj: HandleObject, this: *const Bluetooth<TH>, args: JSJitGetterCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        let this = &*this;
        let result: Option<Rc<dom::bindings::codegen::Bindings::EventHandlerBinding::EventHandlerNonNull<TH>>> = this.GetOnavailabilitychanged();
...
unsafe extern fn set_onavailabilitychanged<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, obj: HandleObject, this: *const Bluetooth<TH>, args: JSJitSetterCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        let this = &*this;
        ...
        let result: () = this.SetOnavailabilitychanged(arg0);
...
fn onavailabilitychanged_getterinfo<TH: TypeHolderTrait<TH>>() -> JSJitInfo {
    JSJitInfo {
...
fn onavailabilitychanged_setterinfo<TH: TypeHolderTrait<TH>>() -> JSJitInfo { JSJitInfo {
...
unsafe extern fn requestDevice<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, _obj: HandleObject, this: *const Bluetooth<TH>, args: *const JSJitMethodCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        let this = &*this;
        let args = &*args;
        let argc = args._base.argc_;
        let arg0: dom::bindings::codegen::Bindings::BluetoothBinding::RequestDeviceOptions = if args.get(0).is_undefined() {
            dom::bindings::codegen::Bindings::BluetoothBinding::RequestDeviceOptions::empty(cx)
        } else {
        ...
        let result: Rc<Promise<TH>> = this.RequestDevice(&arg0);
...
fn requestDevice_methodinfo<TH: TypeHolderTrait<TH>>() -> JSJitInfo { JSJitInfo {
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<Bluetooth<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut Bluetooth<TH>);
            }
            debug!("Bluetooth<TH> finalize: {:p}", this);
    }), ());
}
...
fn Class<TH: TypeHolderTrait<TH>>() -> DOMJSClass { DOMJSClass {
    ...
    dom_class: DOMClass {
    interface_chain: [ PrototypeList::ID::EventTarget, PrototypeList::ID::Bluetooth, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last ],
    ...
    malloc_size_of: malloc_size_of_including_raw_self::<Bluetooth<TH>> as unsafe fn(&mut _, _) -> _,
}
}}

#[inline]
fn malloc_size<TH: TypeHolderTrait<TH>>(ops: &mut MallocSizeOfOps, obj: *const c_void) -> usize {
    malloc_size_of_including_raw_self::<Bluetooth<TH>>(ops, obj)
}

pub unsafe fn Wrap<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<Bluetooth<TH>>) -> DomRoot<Bluetooth<TH>> {
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for Bluetooth<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait<TH>> PartialEq for Bluetooth<TH> {
    fn eq(&self, other: &Bluetooth<TH>) -> bool {
        self as *const Bluetooth<TH> == &*other
    }
}

pub trait BluetoothMethods<TH: TypeHolderTrait<TH>> {
    fn GetAvailability(&self) -> Rc<Promise<TH>>;
    fn GetOnavailabilitychanged(&self) -> Option<Rc<dom::bindings::codegen::Bindings::EventHandlerBinding::EventHandlerNonNull<TH>>>;
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::Bluetooth as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"BluetoothPrototype\0" as *const u8 as *const libc::c_char,
    flags:
...
static INTERFACE_OBJECT_CLASS: NonCallbackInterfaceObjectClass =
    NonCallbackInterfaceObjectClass::new(
        ...
        b"function Bluetooth() {\n    [native code]\n}",
        PrototypeList::ID::Bluetooth,
        1);
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    assert!((*cache)[PrototypeList::ID::Bluetooth as usize].is_null());
    (*cache)[PrototypeList::ID::Bluetooth as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::Bluetooth as isize),
                                  ptr::null_mut(),
                                  prototype.get());
