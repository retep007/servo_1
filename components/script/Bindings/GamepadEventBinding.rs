...
#[derive(JSTraceable)]
pub struct GamepadEventInit<TH: TypeHolderTrait<TH> + 'static> {
    pub parent: dom::bindings::codegen::Bindings::EventBinding::EventInit,
    pub gamepad: DomRoot<Gamepad<TH>>,
}
impl<TH: TypeHolderTrait<TH>> GamepadEventInit<TH> {
    ...
    pub unsafe fn new(cx: *mut JSContext, val: HandleValue)
        ...
        rooted!(in(cx) let object = object);
            let dictionary = GamepadEventInit {
            parent: {
            ...
            gamepad: {
                ...
                match try!(get_dictionary_property(cx, object.handle(), "gamepad", rval.handle_mut())) {
                    true => {
                        if rval.handle().get().is_object() {
                            match root_from_handlevalue(rval.handle()) {
                                Ok(val) => val,
                                Err(()) => {
                                    throw_type_error(cx, "value does not implement interface Gamepad.");
                                    return Err(());
                                }
                            }
                    ...
                    false => {
                        throw_type_error(cx, "Missing required member \"gamepad\".");
                        return Err(());
                    },
                }
            },
...
impl<TH: TypeHolderTrait<TH>> FromJSValConvertible for GamepadEventInit<TH> {
    type Config = ();
    unsafe fn from_jsval(cx: *mut JSContext, value: HandleValue, _option: ())
                         -> Result<ConversionResult<GamepadEventInit<TH>>, ()> {
        GamepadEventInit::new(cx, value)
    }
}

impl<TH: TypeHolderTrait<TH>> ToJSValConvertible for GamepadEventInit<TH> {
    unsafe fn to_jsval(&self, cx: *mut JSContext, mut rval: MutableHandleValue) {
        rooted!(in(cx) let obj = JS_NewObject(cx, ptr::null()));
        let gamepad = &self.gamepad;
        rooted!(in(cx) let mut gamepad_js = UndefinedValue());
        gamepad.to_jsval(cx, gamepad_js.handle_mut());
        set_dictionary_property(cx, obj.handle(), "gamepad", gamepad_js.handle()).unwrap();
        rval.set(ObjectOrNullValue(obj.get()))
    }
}


pub use self::GamepadEventBinding::{Wrap, GamepadEventMethods, GetProtoObject, DefineDOMInterface};
...
unsafe extern fn get_gamepad<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, _obj: HandleObject, this: *const GamepadEvent<TH>, args: JSJitGetterCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        let this = &*this;
        let result: DomRoot<Gamepad<TH>> = this.Gamepad();
...
fn gamepad_getterinfo<TH: TypeHolderTrait<TH>>() -> JSJitInfo { JSJitInfo {
    call: get_gamepad::<TH> as *const os::raw::c_void,
    protoID: PrototypeList::ID::GamepadEvent as u16,
    depth: 1,
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<GamepadEvent<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut GamepadEvent<TH>);
            }
            debug!("GamepadEvent<TH> finalize: {:p}", this);
    }), ());
}
...
fn Class<TH: TypeHolderTrait<TH>>() -> DOMJSClass { DOMJSClass {
    base: js::jsapi::JSClass {
        name: b"GamepadEvent\0" as *const u8 as *const libc::c_char,
    ...
    dom_class: DOMClass {
    interface_chain: [ PrototypeList::ID::Event, PrototypeList::ID::GamepadEvent, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last ],
    type_id: ::dom::bindings::codegen::InheritTypes::TopTypeId { event: (::dom::bindings::codegen::InheritTypes::EventTypeId::GamepadEvent) },
    global: InterfaceObjectMap::Globals::EMPTY,
    malloc_size_of: malloc_size_of_including_raw_self::<GamepadEvent<TH>> as unsafe fn(&mut _, _) -> _,
}
}}

#[inline]
fn malloc_size<TH: TypeHolderTrait<TH>>(ops: &mut MallocSizeOfOps, obj: *const c_void) -> usize {
    malloc_size_of_including_raw_self::<GamepadEvent<TH>>(ops, obj)
}

pub unsafe fn Wrap<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<GamepadEvent<TH>>) -> DomRoot<GamepadEvent<TH>> {
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for GamepadEvent<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait<TH>> PartialEq for GamepadEvent<TH> {
    fn eq(&self, other: &GamepadEvent<TH>) -> bool {
        self as *const GamepadEvent<TH> == &*other
    }
}

pub trait GamepadEventMethods<TH: TypeHolderTrait<TH>> {
...
&[
    JSPropertySpec {
        ...
        getter: JSNativeWrapper { op: Some(generic_getter), info: &gamepad_getterinfo::<TH>() },
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::GamepadEvent as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"GamepadEventPrototype\0" as *const u8 as *const libc::c_char,
    flags:
        // JSCLASS_HAS_RESERVED_SLOTS(1)
        (1 & JSCLASS_RESERVED_SLOTS_MASK) << JSCLASS_RESERVED_SLOTS_SHIFT,
    cOps: 0 as *const _,
...
(cx: *mut JSContext, argc: u32, vp: *mut JSVal) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        if argc < 2 {
            throw_type_error(cx, "Not enough arguments to \"GamepadEvent.constructor\".");
            return false;
        }
        ...
        let arg1: dom::bindings::codegen::Bindings::GamepadEventBinding::GamepadEventInit<TH> = match FromJSValConvertible::from_jsval(cx, args.get(1), ()) {
...
fn INTERFACE_OBJECT_CLASS<TH: TypeHolderTrait<TH>>() -> NonCallbackInterfaceObjectClass {
    ...
    NonCallbackInterfaceObjectClass::new(
        ...
        b"function GamepadEvent() {\n    [native code]\n}",
        PrototypeList::ID::GamepadEvent,
        1) 
}
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    assert!((*cache)[PrototypeList::ID::GamepadEvent as usize].is_null());
    (*cache)[PrototypeList::ID::GamepadEvent as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::GamepadEvent as isize),
                                  ptr::null_mut(),
                                  prototype.get());
