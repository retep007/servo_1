...
pub use self::VRStageParametersBinding::{Wrap, VRStageParametersMethods, GetProtoObject, DefineDOMInterface};
...
unsafe extern fn get_sittingToStandingTransform<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, _obj: HandleObject, this: *const VRStageParameters<TH>, args: JSJitGetterCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        let this = &*this;
        let result: NonNull<JSObject> = this.SittingToStandingTransform(cx);
...
fn sittingToStandingTransform_getterinfo<TH: TypeHolderTrait<TH>>() -> JSJitInfo { JSJitInfo {
...
unsafe extern fn get_sizeX<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, _obj: HandleObject, this: *const VRStageParameters<TH>, args: JSJitGetterCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        let this = &*this;
        let result: Finite<f32> = this.SizeX();
...
fn sizeX_getterinfo<TH: TypeHolderTrait<TH>>() -> JSJitInfo { JSJitInfo {
...
unsafe extern fn get_sizeZ<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, _obj: HandleObject, this: *const VRStageParameters<TH>, args: JSJitGetterCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        let this = &*this;
        let result: Finite<f32> = this.SizeZ();
...
fn sizeZ_getterinfo<TH: TypeHolderTrait<TH>>() -> JSJitInfo { JSJitInfo {
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<VRStageParameters<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut VRStageParameters<TH>);
            }
            debug!("VRStageParameters<TH> finalize: {:p}", this);
    }), ());
}
...
fn Class<TH: TypeHolderTrait<TH>>() -> DOMJSClass { DOMJSClass {
    base: js::jsapi::JSClass {
        name: b"VRStageParameters\0" as *const u8 as *const libc::c_char,
    ...
    dom_class: DOMClass {
    interface_chain: [ PrototypeList::ID::VRStageParameters, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last ],
    type_id: ::dom::bindings::codegen::InheritTypes::TopTypeId { alone: () },
    global: InterfaceObjectMap::Globals::EMPTY,
    malloc_size_of: malloc_size_of_including_raw_self::<VRStageParameters<TH>> as unsafe fn(&mut _, _) -> _,
}
}}

#[inline]
fn malloc_size<TH: TypeHolderTrait<TH>>(ops: &mut MallocSizeOfOps, obj: *const c_void) -> usize {
    malloc_size_of_including_raw_self::<VRStageParameters<TH>>(ops, obj)
}

pub unsafe fn Wrap<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<VRStageParameters<TH>>) -> DomRoot<VRStageParameters<TH>> {
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for VRStageParameters<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait<TH>> PartialEq for VRStageParameters<TH> {
    fn eq(&self, other: &VRStageParameters<TH>) -> bool {
        self as *const VRStageParameters<TH> == &*other
    }
}

pub trait VRStageParametersMethods {
    unsafe fn SittingToStandingTransform(&self, cx: *mut JSContext) -> NonNull<JSObject>;
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::VRStageParameters as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"VRStageParametersPrototype\0" as *const u8 as *const libc::c_char,
    flags:
...
static INTERFACE_OBJECT_CLASS: NonCallbackInterfaceObjectClass =
    NonCallbackInterfaceObjectClass::new(
        ...
        b"function VRStageParameters() {\n    [native code]\n}",
        PrototypeList::ID::VRStageParameters,
        0);
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    assert!((*cache)[PrototypeList::ID::VRStageParameters as usize].is_null());
    (*cache)[PrototypeList::ID::VRStageParameters as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::VRStageParameters as isize),
                                  ptr::null_mut(),
                                  prototype.get());
    ...
    create_noncallback_interface_object(cx,
                                        ...
                                        &[],
                                        prototype.handle(),
                                        b"VRStageParameters\0",
                                        0,
                                        interface.handle_mut());
    assert!(!interface.is_null());
}
} // mod VRStageParametersBinding
