...
fn Class<TH: TypeHolderTrait<TH>>() -> DOMClass { DOMClass {
    ...
    malloc_size_of: malloc_size_of_including_raw_self::<StyleSheetList<TH>> as unsafe fn(&mut _, _) -> _,
}}
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for StyleSheetList<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait<TH>> PartialEq for StyleSheetList<TH> {
    fn eq(&self, other: &StyleSheetList<TH>) -> bool {
        self as *const StyleSheetList<TH> == &*other
    }
}

pub trait StyleSheetListMethods<TH: TypeHolderTrait<TH>> {
