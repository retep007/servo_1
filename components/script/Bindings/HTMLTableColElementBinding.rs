...
pub use self::HTMLTableColElementBinding::{Wrap, GetProtoObject, GetConstructorObject, DefineDOMInterface};
...
use dom::types::HTMLTableColElement;
use dom::windowproxy::WindowProxy;
use js;
use js::JSCLASS_GLOBAL_SLOT_COUNT;
use js::JSCLASS_IS_DOMJSCLASS;
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<HTMLTableColElement<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut HTMLTableColElement<TH>);
            }
            debug!("HTMLTableColElement<TH> finalize: {:p}", this);
    }), ());
}
...
static Class: DOMJSClass = DOMJSClass {
    base: js::jsapi::JSClass {
        name: b"HTMLTableColElement\0" as *const u8 as *const libc::c_char,
    ...
    dom_class: DOMClass {
    interface_chain: [ PrototypeList::ID::EventTarget, PrototypeList::ID::Node, PrototypeList::ID::Element, PrototypeList::ID::HTMLElement, PrototypeList::ID::HTMLTableColElement, PrototypeList::ID::Last ],
...
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<HTMLTableColElement<TH>>) -> DomRoot<HTMLTableColElement<TH>> {
...
impl<TH: TypeHolderTrait> IDLInterface for HTMLTableColElement<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait> PartialEq for HTMLTableColElement<TH> {
    fn eq(&self, other: &HTMLTableColElement<TH>) -> bool {
        self as *const HTMLTableColElement<TH> == &*other
    }
}
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::HTMLTableColElement as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"HTMLTableColElementPrototype\0" as *const u8 as *const libc::c_char,
    flags:
...
(cx: *mut JSContext, argc: u32, vp: *mut JSVal) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let result: Result<DomRoot<HTMLTableColElement<TH>>, Error<TH>> = html_constructor(&global, &args);
...
static INTERFACE_OBJECT_CLASS: NonCallbackInterfaceObjectClass =
    ...
    NonCallbackInterfaceObjectClass::new(
        ...
        b"function HTMLTableColElement() {\n    [native code]\n}",
        PrototypeList::ID::HTMLTableColElement,
        4);
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    /* Check to see whether the interface objects are already installed */
    let proto_or_iface_array = get_proto_or_iface_array(global.get());
    rval.set((*proto_or_iface_array)[PrototypeList::Constructor::HTMLTableColElement as usize]);
    if !rval.get().is_null() {
        return;
    }
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    assert!((*cache)[PrototypeList::ID::HTMLTableColElement as usize].is_null());
    (*cache)[PrototypeList::ID::HTMLTableColElement as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::HTMLTableColElement as isize),
                                  ptr::null_mut(),
                                  prototype.get());
    ...
    create_noncallback_interface_object(cx,
                                        ...
                                        &[],
                                        prototype.handle(),
                                        b"HTMLTableColElement\0",
                                        0,
                                        interface.handle_mut());
    assert!(!interface.is_null());
    assert!((*cache)[PrototypeList::Constructor::HTMLTableColElement as usize].is_null());
    (*cache)[PrototypeList::Constructor::HTMLTableColElement as usize] = interface.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::Constructor::HTMLTableColElement as isize),
                                  ptr::null_mut(),
                                  interface.get());

}
} // mod HTMLTableColElementBinding
