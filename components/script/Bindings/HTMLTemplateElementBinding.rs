...
pub use self::HTMLTemplateElementBinding::{Wrap, HTMLTemplateElementMethods, GetProtoObject, GetConstructorObject, DefineDOMInterface};
...
use dom::types::HTMLTemplateElement;
use dom::types::Node;
use dom::windowproxy::WindowProxy;
use js;
use js::JSCLASS_GLOBAL_SLOT_COUNT;
...
(cx: *mut JSContext, _obj: HandleObject, this: *const HTMLTemplateElement<TH>, args: JSJitGetterCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        let this = &*this;
        let result: DomRoot<DocumentFragment<TH>> = this.Content();
...
const content_getterinfo: JSJitInfo = JSJitInfo {
    ...
    protoID: PrototypeList::ID::HTMLTemplateElement as u16,
    depth: 4,
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<HTMLTemplateElement<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut HTMLTemplateElement<TH>);
            }
            debug!("HTMLTemplateElement<TH> finalize: {:p}", this);
    }), ());
}
...
static Class: DOMJSClass = DOMJSClass {
    base: js::jsapi::JSClass {
        name: b"HTMLTemplateElement\0" as *const u8 as *const libc::c_char,
...
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<HTMLTemplateElement<TH>>) -> DomRoot<HTMLTemplateElement<TH>> {
...
impl<TH: TypeHolderTrait> IDLInterface for HTMLTemplateElement<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait> PartialEq for HTMLTemplateElement<TH> {
    fn eq(&self, other: &HTMLTemplateElement<TH>) -> bool {
        self as *const HTMLTemplateElement<TH> == &*other
    }
}

pub trait HTMLTemplateElementMethods<TH: TypeHolderTrait> {
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::HTMLTemplateElement as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"HTMLTemplateElementPrototype\0" as *const u8 as *const libc::c_char,
    flags:
...
(cx: *mut JSContext, argc: u32, vp: *mut JSVal) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let result: Result<DomRoot<HTMLTemplateElement<TH>>, Error<TH>> = html_constructor(&global, &args);
...
static INTERFACE_OBJECT_CLASS: NonCallbackInterfaceObjectClass =
    ...
    NonCallbackInterfaceObjectClass::new(
        ...
        b"function HTMLTemplateElement() {\n    [native code]\n}",
        PrototypeList::ID::HTMLTemplateElement,
        4);
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    /* Check to see whether the interface objects are already installed */
    let proto_or_iface_array = get_proto_or_iface_array(global.get());
    rval.set((*proto_or_iface_array)[PrototypeList::Constructor::HTMLTemplateElement as usize]);
    if !rval.get().is_null() {
        return;
    }
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    assert!((*cache)[PrototypeList::ID::HTMLTemplateElement as usize].is_null());
    (*cache)[PrototypeList::ID::HTMLTemplateElement as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::HTMLTemplateElement as isize),
                                  ptr::null_mut(),
                                  prototype.get());
    ...
    create_noncallback_interface_object(cx,
                                        ...
                                        &[],
                                        prototype.handle(),
                                        b"HTMLTemplateElement\0",
                                        0,
                                        interface.handle_mut());
    assert!(!interface.is_null());
    assert!((*cache)[PrototypeList::Constructor::HTMLTemplateElement as usize].is_null());
    (*cache)[PrototypeList::Constructor::HTMLTemplateElement as usize] = interface.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::Constructor::HTMLTemplateElement as isize),
                                  ptr::null_mut(),
                                  interface.get());

}
} // mod HTMLTemplateElementBinding
