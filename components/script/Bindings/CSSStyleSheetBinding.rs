...
pub use self::CSSStyleSheetBinding::{Wrap, CSSStyleSheetMethods, GetProtoObject, DefineDOMInterface};
...
(cx: *mut JSContext, _obj: HandleObject, this: *const CSSStyleSheet<TH>, args: *const JSJitMethodCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        if argc < 2 {
            throw_type_error(cx, "Not enough arguments to \"CSSStyleSheet.insertRule\".");
            return false;
        }
...
(cx: *mut JSContext, _obj: HandleObject, this: *const CSSStyleSheet<TH>, args: *const JSJitMethodCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        if argc < 1 {
            throw_type_error(cx, "Not enough arguments to \"CSSStyleSheet.deleteRule\".");
            return false;
        }
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<CSSStyleSheet<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut CSSStyleSheet<TH>);
            }
            debug!("CSSStyleSheet<TH> finalize: {:p}", this);
    }), ());
}
...
fn Class<TH: TypeHolderTrait<TH>>() -> DOMJSClass { DOMJSClass {
    ...
    dom_class: DOMClass {
    interface_chain: [ PrototypeList::ID::StyleSheet, PrototypeList::ID::CSSStyleSheet, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last ],
    ...
    malloc_size_of: malloc_size_of_including_raw_self::<CSSStyleSheet<TH>> as unsafe fn(&mut _, _) -> _,
}
}}

#[inline]
fn malloc_size<TH: TypeHolderTrait<TH>>(ops: &mut MallocSizeOfOps, obj: *const c_void) -> usize {
    malloc_size_of_including_raw_self::<CSSStyleSheet<TH>>(ops, obj)
}

pub unsafe fn Wrap<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<CSSStyleSheet<TH>>) -> DomRoot<CSSStyleSheet<TH>> {
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for CSSStyleSheet<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait<TH>> PartialEq for CSSStyleSheet<TH> {
    fn eq(&self, other: &CSSStyleSheet<TH>) -> bool {
        self as *const CSSStyleSheet<TH> == &*other
    }
}

pub trait CSSStyleSheetMethods<TH: TypeHolderTrait<TH>> {
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::CSSStyleSheet as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"CSSStyleSheetPrototype\0" as *const u8 as *const libc::c_char,
    flags:
...
static INTERFACE_OBJECT_CLASS: NonCallbackInterfaceObjectClass =
    NonCallbackInterfaceObjectClass::new(
        ...
        b"function CSSStyleSheet() {\n    [native code]\n}",
        PrototypeList::ID::CSSStyleSheet,
        1);
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    StyleSheetBinding::GetProtoObject(cx, global, prototype_proto.handle_mut());
    ...
    assert!((*cache)[PrototypeList::ID::CSSStyleSheet as usize].is_null());
    (*cache)[PrototypeList::ID::CSSStyleSheet as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::CSSStyleSheet as isize),
                                  ptr::null_mut(),
                                  prototype.get());


    rooted!(in(cx) let mut interface_proto = ptr::null_mut::<JSObject>());
    StyleSheetBinding::GetConstructorObject(cx, global, interface_proto.handle_mut());
