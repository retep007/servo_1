...
pub mod FrameTypeValues {
    use dom::bindings::conversions::ToJSValConvertible;
    use js::jsapi::JSContext;
    use js::rust::MutableHandleValue;
    use js::jsval::JSVal;

    pub const pairs: &'static [(&'static str, super::FrameType)] = &[
        ("auxiliary", super::FrameType::Auxiliary),
        ("top-level", super::FrameType::Top_level),
        ("nested", super::FrameType::Nested),
        ("none", super::FrameType::None),
    ];

    impl super::FrameType {
        pub fn as_str(&self) -> &'static str {
            pairs[*self as usize].0
        }
    }

    impl Default for super::FrameType {
        fn default() -> super::FrameType {
            pairs[0].1
        }
    }

    impl ToJSValConvertible for super::FrameType {
...
pub use self::ClientBinding::{Wrap, ClientMethods, GetProtoObject, DefineDOMInterface};
...
use dom::bindings::codegen::Bindings::ClientBinding::FrameTypeValues;
use dom::bindings::codegen::InterfaceObjectMap;
use dom::bindings::codegen::PrototypeList;
use dom::bindings::codegen::RegisterBindings;
use dom::bindings::codegen::UnionTypes;
...
(cx: *mut JSContext, _obj: HandleObject, this: *const Client<TH>, args: JSJitGetterCallArgs) -> bool {
...
fn url_getterinfo<TH: TypeHolderTrait<TH>>() -> JSJitInfo { JSJitInfo {
    ...
    protoID: PrototypeList::ID::Client as u16,
    depth: 0,
...
unsafe extern fn get_frameType<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, _obj: HandleObject, this: *const Client<TH>, args: JSJitGetterCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        let this = &*this;
        let result: FrameType = this.FrameType();
...
fn frameType_getterinfo<TH: TypeHolderTrait<TH>>() -> JSJitInfo { JSJitInfo {
    call: get_frameType::<TH> as *const os::raw::c_void,
    protoID: PrototypeList::ID::Client as u16,
    depth: 0,
    _bitfield_1: new_jsjitinfo_bitfield_1!(
        JSJitInfo_OpType::Getter as u8,
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<Client<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut Client<TH>);
            }
            debug!("Client<TH> finalize: {:p}", this);
    }), ());
}
...
fn Class<TH: TypeHolderTrait<TH>>() -> DOMJSClass { DOMJSClass {
    ...
    dom_class: DOMClass {
    interface_chain: [ PrototypeList::ID::Client, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last ],
    type_id: ::dom::bindings::codegen::InheritTypes::TopTypeId { alone: () },
    global: InterfaceObjectMap::Globals::EMPTY,
    malloc_size_of: malloc_size_of_including_raw_self::<Client<TH>> as unsafe fn(&mut _, _) -> _,
}
}}

#[inline]
fn malloc_size<TH: TypeHolderTrait<TH>>(ops: &mut MallocSizeOfOps, obj: *const c_void) -> usize {
    malloc_size_of_including_raw_self::<Client<TH>>(ops, obj)
}

pub unsafe fn Wrap<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<Client<TH>>) -> DomRoot<Client<TH>> {
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for Client<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait<TH>> PartialEq for Client<TH> {
    fn eq(&self, other: &Client<TH>) -> bool {
        self as *const Client<TH> == &*other
    }
}
...
&[
    ...
    JSPropertySpec {
        name: b"frameType\0" as *const u8 as *const libc::c_char,
        flags: (JSPROP_ENUMERATE | JSPROP_SHARED) as u8,
        getter: JSNativeWrapper { op: Some(generic_getter), info: &frameType_getterinfo::<TH>() },
        setter: JSNativeWrapper { op: None, info: 0 as *const JSJitInfo }
    },
    JSPropertySpec {
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::Client as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"ClientPrototype\0" as *const u8 as *const libc::c_char,
    flags:
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    assert!((*cache)[PrototypeList::ID::Client as usize].is_null());
    (*cache)[PrototypeList::ID::Client as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::Client as isize),
                                  ptr::null_mut(),
                                  prototype.get());
