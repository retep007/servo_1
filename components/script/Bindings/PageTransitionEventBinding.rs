...
impl PageTransitionEventInit {
    pub unsafe fn empty(cx: *mut JSContext) -> PageTransitionEventInit {
        match PageTransitionEventInit::new(cx, HandleValue::null()) {
            Ok(ConversionResult::Success(v)) => v,
            _ => unreachable!(),
        }
    }
    pub unsafe fn new(cx: *mut JSContext, val: HandleValue)
                      -> Result<ConversionResult<PageTransitionEventInit>, ()> {
        ...
        rooted!(in(cx) let object = object);
            let dictionary = PageTransitionEventInit {
            parent: {
            ...
            persisted: {
                ...
                match try!(get_dictionary_property(cx, object.handle(), "persisted", rval.handle_mut())) {
                    true => {
...
impl FromJSValConvertible for PageTransitionEventInit {
    ...
    unsafe fn from_jsval(cx: *mut JSContext, value: HandleValue, _option: ())
                         -> Result<ConversionResult<PageTransitionEventInit>, ()> {
        PageTransitionEventInit::new(cx, value)
    }
}
...
impl ToJSValConvertible for PageTransitionEventInit {
    unsafe fn to_jsval(&self, cx: *mut JSContext, mut rval: MutableHandleValue) {
        ...
        rooted!(in(cx) let mut persisted_js = UndefinedValue());
        persisted.to_jsval(cx, persisted_js.handle_mut());
        set_dictionary_property(cx, obj.handle(), "persisted", persisted_js.handle()).unwrap();
        rval.set(ObjectOrNullValue(obj.get()))
    }
}


pub use self::PageTransitionEventBinding::{Wrap, PageTransitionEventMethods, GetProtoObject, DefineDOMInterface};
...
use dom::globalscope::GlobalScope;
use dom::types::Event;
use dom::types::PageTransitionEvent;
use dom::windowproxy::WindowProxy;
use js;
use js::JSCLASS_GLOBAL_SLOT_COUNT;
use js::JSCLASS_IS_DOMJSCLASS;
...
unsafe extern fn get_persisted<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, _obj: HandleObject, this: *const PageTransitionEvent<TH>, args: JSJitGetterCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        let this = &*this;
        let result: bool = this.Persisted();
...
fn persisted_getterinfo<TH: TypeHolderTrait<TH>>() -> JSJitInfo { JSJitInfo {
    call: get_persisted::<TH> as *const os::raw::c_void,
    protoID: PrototypeList::ID::PageTransitionEvent as u16,
    depth: 1,
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<PageTransitionEvent<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut PageTransitionEvent<TH>);
            }
            debug!("PageTransitionEvent<TH> finalize: {:p}", this);
    }), ());
}
...
fn Class<TH: TypeHolderTrait<TH>>() -> DOMJSClass { DOMJSClass {
    base: js::jsapi::JSClass {
        name: b"PageTransitionEvent\0" as *const u8 as *const libc::c_char,
    ...
    dom_class: DOMClass {
    interface_chain: [ PrototypeList::ID::Event, PrototypeList::ID::PageTransitionEvent, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last ],
    type_id: ::dom::bindings::codegen::InheritTypes::TopTypeId { event: (::dom::bindings::codegen::InheritTypes::EventTypeId::PageTransitionEvent) },
    global: InterfaceObjectMap::Globals::EMPTY,
    malloc_size_of: malloc_size_of_including_raw_self::<PageTransitionEvent<TH>> as unsafe fn(&mut _, _) -> _,
}
}}

#[inline]
fn malloc_size<TH: TypeHolderTrait<TH>>(ops: &mut MallocSizeOfOps, obj: *const c_void) -> usize {
    malloc_size_of_including_raw_self::<PageTransitionEvent<TH>>(ops, obj)
}

pub unsafe fn Wrap<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<PageTransitionEvent<TH>>) -> DomRoot<PageTransitionEvent<TH>> {
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for PageTransitionEvent<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait<TH>> PartialEq for PageTransitionEvent<TH> {
    fn eq(&self, other: &PageTransitionEvent<TH>) -> bool {
        self as *const PageTransitionEvent<TH> == &*other
    }
}
...
&[
    JSPropertySpec {
        name: b"persisted\0" as *const u8 as *const libc::c_char,
        flags: (JSPROP_ENUMERATE | JSPROP_SHARED) as u8,
        getter: JSNativeWrapper { op: Some(generic_getter), info: &persisted_getterinfo::<TH>() },
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::PageTransitionEvent as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"PageTransitionEventPrototype\0" as *const u8 as *const libc::c_char,
    flags:
        // JSCLASS_HAS_RESERVED_SLOTS(1)
        (1 & JSCLASS_RESERVED_SLOTS_MASK) << JSCLASS_RESERVED_SLOTS_SHIFT,
    cOps: 0 as *const _,
...
(cx: *mut JSContext, argc: u32, vp: *mut JSVal) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        if argc < 1 {
            throw_type_error(cx, "Not enough arguments to \"PageTransitionEvent.constructor\".");
            return false;
        }
        ...
        let arg1: dom::bindings::codegen::Bindings::PageTransitionEventBinding::PageTransitionEventInit = if args.get(1).is_undefined() {
            dom::bindings::codegen::Bindings::PageTransitionEventBinding::PageTransitionEventInit::empty(cx)
        } else {
...
fn INTERFACE_OBJECT_CLASS<TH: TypeHolderTrait<TH>>() -> NonCallbackInterfaceObjectClass {
    ...
    NonCallbackInterfaceObjectClass::new(
        ...
        b"function PageTransitionEvent() {\n    [native code]\n}",
        PrototypeList::ID::PageTransitionEvent,
        1) 
}
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    assert!((*cache)[PrototypeList::ID::PageTransitionEvent as usize].is_null());
    (*cache)[PrototypeList::ID::PageTransitionEvent as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::PageTransitionEvent as isize),
                                  ptr::null_mut(),
                                  prototype.get());
...
} // mod PageTransitionEventBinding
