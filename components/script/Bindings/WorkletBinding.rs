...
impl WorkletOptions {
    pub unsafe fn empty(cx: *mut JSContext) -> WorkletOptions {
        match WorkletOptions::new(cx, HandleValue::null()) {
            Ok(ConversionResult::Success(v)) => v,
            _ => unreachable!(),
        }
    }
    pub unsafe fn new(cx: *mut JSContext, val: HandleValue)
                      -> Result<ConversionResult<WorkletOptions>, ()> {
        ...
        rooted!(in(cx) let object = object);
            let dictionary = WorkletOptions {
            credentials: {
                rooted!(in(cx) let mut rval = UndefinedValue());
                match try!(get_dictionary_property(cx, object.handle(), "credentials", rval.handle_mut())) {
                    true => {
                        match find_enum_value(cx, rval.handle(), RequestCredentialsValues::pairs) {
                            Err(_) => { return Err(()); },
...
pub use self::WorkletBinding::{Wrap, WorkletMethods, GetProtoObject, DefineDOMInterface};
...
unsafe extern fn addModule<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, _obj: HandleObject, this: *const Worklet<TH>, args: *const JSJitMethodCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        if argc < 1 {
            throw_type_error(cx, "Not enough arguments to \"Worklet.addModule\".");
            return false;
        }
        ...
        let arg1: dom::bindings::codegen::Bindings::WorkletBinding::WorkletOptions = if args.get(1).is_undefined() {
            dom::bindings::codegen::Bindings::WorkletBinding::WorkletOptions::empty(cx)
        } else {
        ...
        let result: Rc<Promise<TH>> = this.AddModule(arg0, &arg1);
...
fn addModule_methodinfo<TH: TypeHolderTrait<TH>>() -> JSJitInfo { JSJitInfo {
    call: addModule::<TH> as *const os::raw::c_void,
    protoID: PrototypeList::ID::Worklet as u16,
    depth: 0,
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<Worklet<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut Worklet<TH>);
            }
            debug!("Worklet<TH> finalize: {:p}", this);
    }), ());
}
...
fn Class<TH: TypeHolderTrait<TH>>() -> DOMJSClass { DOMJSClass {
    base: js::jsapi::JSClass {
        name: b"Worklet\0" as *const u8 as *const libc::c_char,
    ...
    dom_class: DOMClass {
    interface_chain: [ PrototypeList::ID::Worklet, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last ],
    type_id: ::dom::bindings::codegen::InheritTypes::TopTypeId { alone: () },
    global: InterfaceObjectMap::Globals::EMPTY,
    malloc_size_of: malloc_size_of_including_raw_self::<Worklet<TH>> as unsafe fn(&mut _, _) -> _,
}
}}

#[inline]
fn malloc_size<TH: TypeHolderTrait<TH>>(ops: &mut MallocSizeOfOps, obj: *const c_void) -> usize {
    malloc_size_of_including_raw_self::<Worklet<TH>>(ops, obj)
}

pub unsafe fn Wrap<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<Worklet<TH>>) -> DomRoot<Worklet<TH>> {
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for Worklet<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait<TH>> PartialEq for Worklet<TH> {
    fn eq(&self, other: &Worklet<TH>) -> bool {
        self as *const Worklet<TH> == &*other
    }
}

pub trait WorkletMethods<TH: TypeHolderTrait<TH>> {
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::Worklet as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"WorkletPrototype\0" as *const u8 as *const libc::c_char,
    flags:
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    assert!((*cache)[PrototypeList::ID::Worklet as usize].is_null());
    (*cache)[PrototypeList::ID::Worklet as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::Worklet as isize),
                                  ptr::null_mut(),
                                  prototype.get());
