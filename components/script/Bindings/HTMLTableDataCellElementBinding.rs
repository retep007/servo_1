...
pub use self::HTMLTableDataCellElementBinding::{Wrap, GetProtoObject, GetConstructorObject, DefineDOMInterface};
...
use dom::types::HTMLTableDataCellElement;
use dom::windowproxy::WindowProxy;
use js;
use js::JSCLASS_GLOBAL_SLOT_COUNT;
use js::JSCLASS_IS_DOMJSCLASS;
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<HTMLTableDataCellElement<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut HTMLTableDataCellElement<TH>);
            }
            debug!("HTMLTableDataCellElement<TH> finalize: {:p}", this);
    }), ());
}
...
fn Class<TH: TypeHolderTrait<TH>>() -> DOMJSClass { DOMJSClass {
    base: js::jsapi::JSClass {
        name: b"HTMLTableDataCellElement\0" as *const u8 as *const libc::c_char,
    ...
    dom_class: DOMClass {
    interface_chain: [ PrototypeList::ID::EventTarget, PrototypeList::ID::Node, PrototypeList::ID::Element, PrototypeList::ID::HTMLElement, PrototypeList::ID::HTMLTableCellElement, PrototypeList::ID::HTMLTableDataCellElement ],
    ...
    global: InterfaceObjectMap::Globals::EMPTY,
   	malloc_size_of: malloc_size_of_including_raw_self::<HTMLTableDataCellElement<TH>> as unsafe fn(&mut _, _) -> _,
}
}}

#[inline]
fn malloc_size<TH: TypeHolderTrait<TH>>(ops: &mut MallocSizeOfOps, obj: *const c_void) -> usize {
    malloc_size_of_including_raw_self::<HTMLTableDataCellElement<TH>>(ops, obj)
}

pub unsafe fn Wrap<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<HTMLTableDataCellElement<TH>>) -> DomRoot<HTMLTableDataCellElement<TH>> {
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for HTMLTableDataCellElement<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait<TH>> PartialEq for HTMLTableDataCellElement<TH> {
    fn eq(&self, other: &HTMLTableDataCellElement<TH>) -> bool {
        self as *const HTMLTableDataCellElement<TH> == &*other
    }
}
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::HTMLTableDataCellElement as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"HTMLTableDataCellElementPrototype\0" as *const u8 as *const libc::c_char,
    flags:
...
(cx: *mut JSContext, argc: u32, vp: *mut JSVal) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let result: Result<DomRoot<HTMLTableDataCellElement<TH>>, Error<TH>> = html_constructor(&global, &args);
...
fn INTERFACE_OBJECT_CLASS<TH: TypeHolderTrait<TH>>() -> NonCallbackInterfaceObjectClass {
    ...
    NonCallbackInterfaceObjectClass::new(
        ...
        b"function HTMLTableDataCellElement() {\n    [native code]\n}",
        PrototypeList::ID::HTMLTableDataCellElement,
        5) 
}
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    /* Check to see whether the interface objects are already installed */
    let proto_or_iface_array = get_proto_or_iface_array(global.get());
    rval.set((*proto_or_iface_array)[PrototypeList::Constructor::HTMLTableDataCellElement as usize]);
    if !rval.get().is_null() {
        return;
    }
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    assert!((*cache)[PrototypeList::ID::HTMLTableDataCellElement as usize].is_null());
    (*cache)[PrototypeList::ID::HTMLTableDataCellElement as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::HTMLTableDataCellElement as isize),
                                  ptr::null_mut(),
                                  prototype.get());
    ...
    create_noncallback_interface_object(cx,
                                        ...
                                        &[],
                                        prototype.handle(),
                                        b"HTMLTableDataCellElement\0",
                                        0,
                                        interface.handle_mut());
    assert!(!interface.is_null());
    assert!((*cache)[PrototypeList::Constructor::HTMLTableDataCellElement as usize].is_null());
    (*cache)[PrototypeList::Constructor::HTMLTableDataCellElement as usize] = interface.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::Constructor::HTMLTableDataCellElement as isize),
                                  ptr::null_mut(),
                                  interface.get());

}
} // mod HTMLTableDataCellElementBinding
