...
pub use self::MimeTypeArrayBinding::{Wrap, MimeTypeArrayMethods, GetProtoObject, DefineDOMInterface};
...
(cx: *mut JSContext, _obj: HandleObject, this: *const MimeTypeArray<TH>, args: JSJitGetterCallArgs) -> bool {
...
fn length_getterinfo<TH: TypeHolderTrait<TH>>() -> JSJitInfo { JSJitInfo {
    ...
    protoID: PrototypeList::ID::MimeTypeArray as u16,
    depth: 0,
...
(cx: *mut JSContext, _obj: HandleObject, this: *const MimeTypeArray<TH>, args: *const JSJitMethodCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        let this = &*this;
        let args = &*args;
        ...
        if argc < 1 {
            throw_type_error(cx, "Not enough arguments to \"MimeTypeArray.item\".");
            return false;
        }
...
(cx: *mut JSContext, _obj: HandleObject, this: *const MimeTypeArray<TH>, args: *const JSJitMethodCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        if argc < 1 {
            throw_type_error(cx, "Not enough arguments to \"MimeTypeArray.namedItem\".");
            return false;
        }
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<MimeTypeArray<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut MimeTypeArray<TH>);
            }
            debug!("MimeTypeArray<TH> finalize: {:p}", this);
    }), ());
}
...
(obj: RawHandleObject) -> *const MimeTypeArray<TH> {
    /*if (xpc::WrapperFactory::IsXrayWrapper(obj)) {
        obj = js::UnwrapObject(obj);
    }*/
    //MOZ_ASSERT(IsProxy(obj));
    let box_ = GetProxyPrivate(obj.get()).to_private() as *const MimeTypeArray<TH>;
    return box_;
}

fn Class<TH: TypeHolderTrait<TH>>() -> DOMClass { DOMClass {
    interface_chain: [ PrototypeList::ID::MimeTypeArray, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last ],
    type_id: ::dom::bindings::codegen::InheritTypes::TopTypeId { alone: () },
    global: InterfaceObjectMap::Globals::EMPTY,
    malloc_size_of: malloc_size_of_including_raw_self::<MimeTypeArray<TH>> as unsafe fn(&mut _, _) -> _,
}}
...
(cx: *mut JSContext, _proxy: RawHandleObject) -> *const i8 {
    b"MimeTypeArray\0" as *const u8 as *const i8
}
...
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<MimeTypeArray<TH>>) -> DomRoot<MimeTypeArray<TH>> {
    ...
    let handler = RegisterBindings::PROXY_HANDLERS[PrototypeList::Proxies::MimeTypeArray as usize];
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for MimeTypeArray<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait<TH>> PartialEq for MimeTypeArray<TH> {
    fn eq(&self, other: &MimeTypeArray<TH>) -> bool {
        self as *const MimeTypeArray<TH> == &*other
    }
}

pub trait MimeTypeArrayMethods<TH: TypeHolderTrait<TH>> {
    fn Length(&self) -> u32;
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::MimeTypeArray as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"MimeTypeArrayPrototype\0" as *const u8 as *const libc::c_char,
    flags:
...
static INTERFACE_OBJECT_CLASS: NonCallbackInterfaceObjectClass =
    NonCallbackInterfaceObjectClass::new(
        ...
        b"function MimeTypeArray() {\n    [native code]\n}",
        PrototypeList::ID::MimeTypeArray,
        0);
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    assert!((*cache)[PrototypeList::ID::MimeTypeArray as usize].is_null());
    (*cache)[PrototypeList::ID::MimeTypeArray as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::MimeTypeArray as isize),
                                  ptr::null_mut(),
                                  prototype.get());
