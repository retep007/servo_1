...
pub use self::OESStandardDerivativesBinding::{OESStandardDerivativesConstants, Wrap, GetProtoObject};
...
use dom::bindings::weakref::WeakReferenceable;
use dom::globalscope::GlobalScope;
use dom::types::OESStandardDerivatives;
use dom::windowproxy::WindowProxy;
use js;
use js::JSCLASS_GLOBAL_SLOT_COUNT;
use js::JSCLASS_IS_DOMJSCLASS;
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<OESStandardDerivatives<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut OESStandardDerivatives<TH>);
            }
            debug!("OESStandardDerivatives<TH> finalize: {:p}", this);
    }), ());
}
...
pub mod OESStandardDerivativesConstants {
    pub const FRAGMENT_SHADER_DERIVATIVE_HINT_OES: u32 = 35723;
} // mod OESStandardDerivativesConstants
...
fn Class<TH: TypeHolderTrait<TH>>() -> DOMJSClass { DOMJSClass {
    base: js::jsapi::JSClass {
        name: b"OESStandardDerivatives\0" as *const u8 as *const libc::c_char,
    ...
    dom_class: DOMClass {
    interface_chain: [ PrototypeList::ID::OESStandardDerivatives, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last ],
    type_id: ::dom::bindings::codegen::InheritTypes::TopTypeId { alone: () },
    global: InterfaceObjectMap::Globals::EMPTY,
    malloc_size_of: malloc_size_of_including_raw_self::<OESStandardDerivatives<TH>> as unsafe fn(&mut _, _) -> _,
}
}}

#[inline]
fn malloc_size<TH: TypeHolderTrait<TH>>(ops: &mut MallocSizeOfOps, obj: *const c_void) -> usize {
    malloc_size_of_including_raw_self::<OESStandardDerivatives<TH>>(ops, obj)
}

pub unsafe fn Wrap<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<OESStandardDerivatives<TH>>) -> DomRoot<OESStandardDerivatives<TH>> {
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for OESStandardDerivatives<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait<TH>> PartialEq for OESStandardDerivatives<TH> {
    fn eq(&self, other: &OESStandardDerivatives<TH>) -> bool {
        self as *const OESStandardDerivatives<TH> == &*other
    }
}

const sConstants_specs: &'static [&'static[ConstantSpec]] = &[
&[
    ConstantSpec { name: b"FRAGMENT_SHADER_DERIVATIVE_HINT_OES\0", value: ConstantVal::UintVal(35723) }]
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::OESStandardDerivatives as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"OESStandardDerivativesPrototype\0" as *const u8 as *const libc::c_char,
    flags:
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    assert!((*cache)[PrototypeList::ID::OESStandardDerivatives as usize].is_null());
    (*cache)[PrototypeList::ID::OESStandardDerivatives as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::OESStandardDerivatives as isize),
                                  ptr::null_mut(),
                                  prototype.get());

}
} // mod OESStandardDerivativesBinding
