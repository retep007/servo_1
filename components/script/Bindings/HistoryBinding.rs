...
pub trait HistoryMethods<TH: TypeHolderTrait> {
    ...
    unsafe fn GetState(&self, cx: *mut JSContext) -> Fallible<JSVal>;
    fn Go(&self, delta: i32) -> Fallible<()>;
    fn Back(&self) -> Fallible<()>;
    fn Forward(&self) -> Fallible<()>;
    unsafe fn PushState(&self, cx: *mut JSContext, data: HandleValue, title: DOMString, url: Option<USVString>) -> Fallible<()>;
    unsafe fn ReplaceState(&self, cx: *mut JSContext, data: HandleValue, title: DOMString, url: Option<USVString>) -> Fallible<()>;
}
