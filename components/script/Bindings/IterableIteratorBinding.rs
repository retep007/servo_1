...
impl IterableKeyOrValueResult {
    ...
    pub unsafe fn new(cx: *mut JSContext, val: HandleValue)
        ...
        rooted!(in(cx) let object = object);
            let mut dictionary = RootedTraceableBox::new(IterableKeyOrValueResult::default());
            dictionary.done = {
                rooted!(in(cx) let mut rval = UndefinedValue());
                match try!(get_dictionary_property(cx, object.handle(), "done", rval.handle_mut())) {
                    true => {
            ...
            dictionary.value = {
                rooted!(in(cx) let mut rval = UndefinedValue());
                match try!(get_dictionary_property(cx, object.handle(), "value", rval.handle_mut())) {
                    true => {
...
impl FromJSValConvertible for RootedTraceableBox<IterableKeyOrValueResult> {
    ...
    unsafe fn from_jsval(cx: *mut JSContext, value: HandleValue, _option: ())
                         -> Result<ConversionResult<RootedTraceableBox<IterableKeyOrValueResult>>, ()> {
        IterableKeyOrValueResult::new(cx, value)
    }
}

impl ToJSValConvertible for RootedTraceableBox<IterableKeyOrValueResult> {
...
impl IterableKeyAndValueResult {
    ...
    pub unsafe fn new(cx: *mut JSContext, val: HandleValue)
        ...
        rooted!(in(cx) let object = object);
            let mut dictionary = RootedTraceableBox::new(IterableKeyAndValueResult::default());
            dictionary.done = {
                rooted!(in(cx) let mut rval = UndefinedValue());
                match try!(get_dictionary_property(cx, object.handle(), "done", rval.handle_mut())) {
                    true => {
...
impl FromJSValConvertible for RootedTraceableBox<IterableKeyAndValueResult> {
    ...
    unsafe fn from_jsval(cx: *mut JSContext, value: HandleValue, _option: ())
                         -> Result<ConversionResult<RootedTraceableBox<IterableKeyAndValueResult>>, ()> {
        IterableKeyAndValueResult::new(cx, value)
    }
}

impl ToJSValConvertible for RootedTraceableBox<IterableKeyAndValueResult> {
    unsafe fn to_jsval(&self, cx: *mut JSContext, mut rval: MutableHandleValue) {
        rooted!(in(cx) let obj = JS_NewObject(cx, ptr::null()));
        let done = &self.done;
        rooted!(in(cx) let mut done_js = UndefinedValue());
