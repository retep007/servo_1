...
pub use self::HTMLBodyElementBinding::{Wrap, HTMLBodyElementMethods, GetProtoObject, GetConstructorObject, DefineDOMInterface};
...
(cx: *mut JSContext, _obj: HandleObject, this: *const HTMLBodyElement<TH>, args: JSJitGetterCallArgs) -> bool {
...
(cx: *mut JSContext, obj: HandleObject, this: *const HTMLBodyElement<TH>, args: JSJitSetterCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        let this = &*this;
...
fn text_getterinfo<TH: TypeHolderTrait<TH>>() -> JSJitInfo { JSJitInfo {
    ...
    protoID: PrototypeList::ID::HTMLBodyElement as u16,
    depth: 4,
...
(cx: *mut JSContext, obj: HandleObject, this: *const HTMLBodyElement<TH>, args: JSJitSetterCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let result: () = this.SetBackground(arg0);
        pop_current_element_queue::<TH>();
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<HTMLBodyElement<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut HTMLBodyElement<TH>);
            }
            debug!("HTMLBodyElement<TH> finalize: {:p}", this);
    }), ());
}
...
fn Class<TH: TypeHolderTrait<TH>>() -> DOMJSClass { DOMJSClass {
    base: js::jsapi::JSClass {
        name: b"HTMLBodyElement\0" as *const u8 as *const libc::c_char,
    ...
    dom_class: DOMClass {
    interface_chain: [ PrototypeList::ID::EventTarget, PrototypeList::ID::Node, PrototypeList::ID::Element, PrototypeList::ID::HTMLElement, PrototypeList::ID::HTMLBodyElement, PrototypeList::ID::Last ],
    ...
    malloc_size_of: malloc_size_of_including_raw_self::<HTMLBodyElement<TH>> as unsafe fn(&mut _, _) -> _,
}
}}

#[inline]
fn malloc_size<TH: TypeHolderTrait<TH>>(ops: &mut MallocSizeOfOps, obj: *const c_void) -> usize {
    malloc_size_of_including_raw_self::<HTMLBodyElement<TH>>(ops, obj)
}

pub unsafe fn Wrap<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<HTMLBodyElement<TH>>) -> DomRoot<HTMLBodyElement<TH>> {
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for HTMLBodyElement<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait<TH>> PartialEq for HTMLBodyElement<TH> {
    fn eq(&self, other: &HTMLBodyElement<TH>) -> bool {
        self as *const HTMLBodyElement<TH> == &*other
    }
}

pub trait HTMLBodyElementMethods<TH: TypeHolderTrait<TH>> {
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::HTMLBodyElement as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"HTMLBodyElementPrototype\0" as *const u8 as *const libc::c_char,
    flags:
...
(cx: *mut JSContext, argc: u32, vp: *mut JSVal) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let result: Result<DomRoot<HTMLBodyElement<TH>>, Error<TH>> = html_constructor(&global, &args);
...
fn INTERFACE_OBJECT_CLASS<TH: TypeHolderTrait<TH>>() -> NonCallbackInterfaceObjectClass {
    ...
    NonCallbackInterfaceObjectClass::new(
        ...
        b"function HTMLBodyElement() {\n    [native code]\n}",
        PrototypeList::ID::HTMLBodyElement,
        4) 
}
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    /* Check to see whether the interface objects are already installed */
    let proto_or_iface_array = get_proto_or_iface_array(global.get());
    rval.set((*proto_or_iface_array)[PrototypeList::Constructor::HTMLBodyElement as usize]);
    if !rval.get().is_null() {
        return;
    }
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    assert!((*cache)[PrototypeList::ID::HTMLBodyElement as usize].is_null());
    (*cache)[PrototypeList::ID::HTMLBodyElement as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::HTMLBodyElement as isize),
                                  ptr::null_mut(),
                                  prototype.get());
    ...
    assert!((*cache)[PrototypeList::Constructor::HTMLBodyElement as usize].is_null());
    (*cache)[PrototypeList::Constructor::HTMLBodyElement as usize] = interface.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::Constructor::HTMLBodyElement as isize),
                                  ptr::null_mut(),
                                  interface.get());

}
} // mod HTMLBodyElementBinding
