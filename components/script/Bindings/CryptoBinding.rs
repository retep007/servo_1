...
pub use self::CryptoBinding::{Wrap, CryptoMethods, GetProtoObject, DefineDOMInterface};
...
unsafe extern fn getRandomValues<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, _obj: HandleObject, this: *const Crypto<TH>, args: *const JSJitMethodCallArgs) -> bool {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        if argc < 1 {
            throw_type_error(cx, "Not enough arguments to \"Crypto.getRandomValues\".");
            return false;
        }
...
fn getRandomValues_methodinfo<TH: TypeHolderTrait<TH>>() -> JSJitInfo { JSJitInfo {
    call: getRandomValues::<TH> as *const os::raw::c_void,
    protoID: PrototypeList::ID::Crypto as u16,
    depth: 0,
...
(_fop: *mut JSFreeOp, obj: *mut JSObject) {
    return wrap_panic(panic::AssertUnwindSafe(|| {
        ...
        let this = native_from_object::<Crypto<TH>>(obj).unwrap();
            if !this.is_null() {
                // The pointer can be null if the object is the unforgeable holder of that interface.
                let _ = Box::from_raw(this as *mut Crypto<TH>);
            }
            debug!("Crypto<TH> finalize: {:p}", this);
    }), ());
}
...
fn Class<TH: TypeHolderTrait<TH>>() -> DOMJSClass { DOMJSClass {
    ...
    dom_class: DOMClass {
    interface_chain: [ PrototypeList::ID::Crypto, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last, PrototypeList::ID::Last ],
    type_id: ::dom::bindings::codegen::InheritTypes::TopTypeId { alone: () },
    global: InterfaceObjectMap::Globals::EMPTY,
    malloc_size_of: malloc_size_of_including_raw_self::<Crypto<TH>> as unsafe fn(&mut _, _) -> _,
}
}}

#[inline]
fn malloc_size<TH: TypeHolderTrait<TH>>(ops: &mut MallocSizeOfOps, obj: *const c_void) -> usize {
    malloc_size_of_including_raw_self::<Crypto<TH>>(ops, obj)
}

pub unsafe fn Wrap<TH: TypeHolderTrait<TH>>
(cx: *mut JSContext, scope: &GlobalScope<TH>, object: Box<Crypto<TH>>) -> DomRoot<Crypto<TH>> {
...
impl<TH: TypeHolderTrait<TH>> IDLInterface for Crypto<TH> {
    #[inline]
...
impl<TH: TypeHolderTrait<TH>> PartialEq for Crypto<TH> {
    fn eq(&self, other: &Crypto<TH>) -> bool {
        self as *const Crypto<TH> == &*other
    }
}

pub trait CryptoMethods<TH: TypeHolderTrait<TH>> {
...
&[
    JSFunctionSpec {
        name: b"getRandomValues\0" as *const u8 as *const libc::c_char,
        call: JSNativeWrapper { op: Some(generic_method), info: &getRandomValues_methodinfo::<TH>() as *const _ as *const JSJitInfo },
        nargs: 1,
...
(cx: *mut JSContext, global: HandleObject, mut rval: MutableHandleObject) {
    ...
    rval.set((*proto_or_iface_array)[PrototypeList::ID::Crypto as usize]);
    if !rval.get().is_null() {
        return;
    }
...
static PrototypeClass: JSClass = JSClass {
    name: b"CryptoPrototype\0" as *const u8 as *const libc::c_char,
    flags:
...
(cx: *mut JSContext, global: HandleObject, cache: *mut ProtoOrIfaceArray) {
    ...
    assert!((*cache)[PrototypeList::ID::Crypto as usize].is_null());
    (*cache)[PrototypeList::ID::Crypto as usize] = prototype.get();
    <*mut JSObject>::post_barrier((*cache).as_mut_ptr().offset(PrototypeList::ID::Crypto as isize),
                                  ptr::null_mut(),
                                  prototype.get());
